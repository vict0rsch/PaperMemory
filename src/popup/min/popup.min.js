...
${o}</textarea></div></div><div style="display: flex; justify-content: space-between; align-items: center"><div style="display: flex; justify-content: flex-start; align-items: center"><label class="label" for="checkFavorite">Favorite:</label> <input ${""} class="switch" type="checkbox" id="checkFavorite--${t}" name="checkFavorite" value="checkFavorite"></div><div id="popup-delete-paper" title="Delete paper from Memory"><svg width="25" height="25" viewBox="0 0 24 24" stroke-width="1" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke="white"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><line x1="4" y1="7" x2="20" y2="7"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/><path d="M5 7l1 12a2 2 0 0 0 2 2h8a2 2 0 0 0 2 -2l1 -12"/><path d="M9 7v-3a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v3"/></svg></div><small id="popup-displayId">${s} </small><button hidden class="back-to-focus" id="popup-save-edits--${t}">Save</button></div></div></div>`},getPopupPaperIconsHTML=(e,t,a)=>{var o=e.id,t=isPdfUrl(t)?"HTML":"PDF";let s="";global.state.menu.checkScirate&&"arxiv"===e.source&&(s=`<div tabindex="0" class="memory-item-svg-div" id="popup-memory-item-scirate--${o}" title="Open on SciRate">${tablerSvg("messages","",["popup-click-svg"])}</div>`);e=global.state.menu.checkStore&&(a.localFile||a.stored)?`<div tabindex="0" class="memory-item-svg-div" id="popup-memory-item-openLocal--${o}" title="Open downloaded pdf">${tablerSvg("vocabulary","",["popup-click-svg"])}</div>`:`<div tabindex="0" class="memory-item-svg-div" id="popup-memory-item-download--${o}" title="Download pdf">${tablerSvg("file-download","",["popup-click-svg"])}</div>`;return`${s}<div tabindex="0" class="memory-item-svg-div" id="popup-memory-item-link--${o}" title="Open Paper ${t} Page">${tablerSvg("external-link","",["popup-click-svg"])}</div><div tabindex="0" class="memory-item-svg-div" id="popup-memory-item-copy-link--${o}" title="Copy link to paper">${tablerSvg("link","",["popup-click-svg"])}</div><div tabindex="0" class="memory-item-svg-div" id="popup-memory-item-md--${o}" title="Copy Markdown-formatted link">${tablerSvg("markdown","",["popup-click-svg"])}</div><div tabindex="0" class="memory-item-svg-div" id="popup-memory-item-bibtex--${o}" title="Copy Bibtex citation">${tablerSvg("math-function","",["popup-click-svg"])}</div>`+e},getTagsOptions=e=>{const o=new Set(e.tags);return Array.from(global.state.paperTags).sort().map((e,t)=>{let a='<option value="'+e+'"';return o.has(e)&&(a+=' selected="selected" '),a+">"+e}).join("")},updateAllMemoryPaperTagOptions=()=>{for(const t in global.state.papers){var e;global.state.papers.hasOwnProperty(t)&&"__dataVersion"!==t&&(e=global.state.papers[t],setHTML("memory-item-tags--"+t,getTagsOptions(e)))}},sampleAsciiArt=async()=>{var e=chrome.runtime.getURL("src/popup/art.json"),e=await fetch(e).then(e=>e.json()),t=Object.keys(e).length,t=Math.floor(Math.random()*t),[e,t]=Object.entries(e)[t];return{animal:e,ascii:t}},updatePopupPaperNoMemory=async o=>{var{animal:e,ascii:t}=await sampleAsciiArt();let a=`<div class="no-paper-div"><h3>This paper is not in your Memory  <span id="no-paper-why-span"><code id="no-paper-why-code">?</code></span></h3><div><div>Here's a ${e} for your trouble</div><div id="ascii-art-div"><div style="text-align:">${t}</div></div></div></div>`;e=-1<navigator.userAgent.search("Firefox")||global.state.menu.checkNoAuto;e&&(a+='<div id="manual-trigger-wrapper"><div id="manual-trigger-btn">Try manual trigger</div><div id="manual-loader-container" class="arxivTools-container" style="display: none;"><div class="sk-folding-cube"><div class="sk-cube1 sk-cube"></div><div class="sk-cube2 sk-cube"></div><div class="sk-cube4 sk-cube"></div><div class="sk-cube3 sk-cube"></div></div></div><div id="manual-parsing-error"></div></div>');const s=findEl("isArxiv").innerHTML;setHTML("isArxiv",a),addListener("no-paper-why-code","click",()=>{showPopupModal("noPaper")}),e&&addListener("manual-trigger-btn","click",async()=>{showId("manual-loader-container");try{var t=await isPaper(o);let e;e.ndzeij.dz();var a=await addOrUpdatePaper(o,t);if(!a)return;(e=a.paper)&&(hideId("manual-loader-container"),setHTML("isArxiv",s),popupMain(o,t,!0))}catch(e){hideId("manual-loader-container"),setHTML("manual-parsing-error",`<strong>${"There was an issue parsing this paper. <br/> Raise an issue on Github if you think it is a bug.<br/>Attempted url: "+o}</strong>`),warn("Manual Parsing Error:",e)}})},showConfirmDeleteModal=e=>{var t=global.state.papers[e].title;setTextId("delete-modal-title",t),setHTML("hidden-modal-id",e),showId("confirm-modal","flex")},copyAndConfirmMemoryItem=(e,t,a,o)=>{copyTextToClipboard(t);const s=o?findEl("popup-feedback-copied"):findEl(e,"memory-item-feedback");s&&(s.innerText=a,fadeIn(s),setTimeout(()=>{fadeOut(s)},2e3))},focusExistingOrCreateNewCodeTab=s=>{(s=s.replace("http://","https://")).startsWith("https://")||(s="https://"+s);var e=new URL(s)["origin"];chrome.tabs.query({url:e+"/*"},e=>{for(const t of e)if(t.url.includes(s)){const a={active:!0},o={focused:!0};return void chrome.windows.getCurrent(e=>{e.id!==t.windowId?chrome.windows.update(t.windowId,o,()=>{chrome.tabs.update(t.id,a)}):chrome.tabs.update(t.id,a)})}chrome.tabs.create({url:s})})},focusExistingOrCreateNewPaperTab=(l,i)=>{chrome.tabs.query({},async e=>{var t=global.state.menu;let a=[];for(const r of e){let e;try{e=r.url&&await parseIdFromUrl(r.url)}catch(e){}e&&e===l.id&&a.push(r)}let o;var s,e=t.checkPreferPdf?a.filter(e=>e.url&&isPdfUrl(e.url)):a.filter(e=>e.url&&!isPdfUrl(e.url));0<e.length?(s=i&&global.state.files.hasOwnProperty(l.id)?[]:a.filter(e=>e.url.startsWith("file://")),o=(0<s.length?s:e)[0]):0<a.length&&(o=a[0]),o?chrome.windows.getCurrent(e=>{e.id!==o.windowId?chrome.windows.update(o.windowId,{focused:!0},()=>{chrome.tabs.update(o.id,{active:!0})}):chrome.tabs.update(o.id,{active:!0})}):global.state.files.hasOwnProperty(l.id)&&!i?chrome.downloads.open(global.state.files[l.id].id):chrome.tabs.create({url:(t.checkPreferPdf?paperToPDF:paperToAbs)(l)}),global.state.papers[l.id].count+=1,chrome.storage.local.set({papers:global.state.papers})})},saveNote=(t,a)=>{global.state.papers[t].note=a,chrome.storage.local.set({papers:global.state.papers},()=>{setHTML(findEl(t,"memory-note-div"),a?`<div class="memory-note-div memory-item-faded"><span class="note-content-header">Note:</span> <span class="note-content">${a}</span></div>`:'<div class="memory-note-div memory-item-faded"></div>');var e=findEl("popup-form-note-textarea--"+t);val(e,a),val(findEl(t,"form-note-textarea"),a)})},saveCodeLink=(t,a)=>{a=a.trim(),global.state.papers[t].codeLink=a,chrome.storage.local.set({papers:global.state.papers},()=>{var e=a.replace(/^https?:\/\//,""),e=(setHTML(findEl(t,"memory-code-link"),e),setHTML("popup-code-link",e),val(findEl(t,"form-code-input"),a),(a?showId:hideId)("popup-code-link"),findEl("popup-form-codeLink--"+t));val(e,a)})},saveFavoriteItem=(o,s)=>{global.state.papers[o].favorite=s,global.state.papers[o].favoriteDate=(new Date).toJSON(),chrome.storage.local.set({papers:global.state.papers},()=>{var e,t;s?(addClass("memory-container--"+o,"favorite"),addClass(findEl(o,"memory-item-favorite").querySelector("svg"),"favorite")):(removeClass("memory-container--"+o,"favorite"),removeClass(findEl(o,"memory-item-favorite").querySelector("svg"),"favorite")),"favoriteDate"===global.state.sortKey&&(s||(sortMemory(),displayMemoryTable()),e=global.state.sortedPapers.filter(e=>e.favorite).length,(t=findEl("memory-search"))&&setPlaceholder(t,`Search ${e} entries`));let a=findEl("checkFavorite--"+o);a&&(a.checked=s)})},setMemorySortArrow=e=>{let t;t="up"===e?'<svg viewBox="0 0 24 24" class="memory-sort-arrow-svg" id="memory-sort-arrow-up"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><line x1="12" y1="5" x2="12" y2="19"/><line x1="16" y1="9" x2="12" y2="5"/><line x1="8" y1="9" x2="12" y2="5"/></svg>':'<svg class="memory-sort-arrow-svg" id="memory-sort-arrow-down" viewBox="0 0 24 24"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><line x1="12" y1="5" x2="12" y2="19"/><line x1="16" y1="15" x2="12" y2="19"/><line x1="8" y1="15" x2="12" y2="19"/></svg>',setHTML("memory-sort-arrow",t)},orderPapers=(e,t)=>{let a=e[global.state.sortKey],o=t[global.state.sortKey];return void 0===a&&(a=""),void 0===o&&(o=""),"string"==typeof a&&(a=a.toLowerCase(),o=o.toLowerCase()),0<=global.descendingSortKeys.indexOf(global.state.sortKey)?a>o?-1:1:a>o?1:-1},sortMemory=()=>{global.state.sortedPapers=Object.values(cleanPapers(global.state.papers)),global.state.sortedPapers.sort(orderPapers),global.state.papersList.sort(orderPapers)},reverseMemory=()=>{global.state.sortedPapers.reverse(),global.state.papersList.reverse()},searchMemory=e=>{const t=e.toLowerCase().split(" ");let a=[];for(const o of global.state.sortedPapers){const s=o.title.toLowerCase(),r=o.author.toLowerCase(),l=o.note.toLowerCase(),i=o.tags.join(" ").toLowerCase(),n=getDisplayId(o.id).toLowerCase();!t.every(e=>s.includes(e)||r.includes(e)||l.includes(e)||i.includes(e)||n.includes(e))||global.state.showFavorites&&!o.favorite||a.push(o)}global.state.papersList=a},searchMemoryByYear=e=>{var t=e.includes("<")?"smaller":e.includes(">")?"greater":"";const a=e.replace("y:","").replace(/(<|>)/g,"").toLowerCase().replaceAll(","," ").split(" ").filter(e=>0<e.length).map(e=>4===e.length?e:"20"+e).map(e=>parseInt(e,10));console.log("searchYears: ",a);let o=[],s=(e,t)=>e===t;"smaller"==t?s=(e,t)=>t<e:"greater"==t&&(s=(e,t)=>e<t);for(const r of global.state.sortedPapers){const l=parseInt(r.year,10);a.some(e=>s(e,l))&&o.push(r)}global.state.papersList=o},searchMemoryByTags=e=>{const t=e.replace("t:","").toLowerCase().split(" ");let a=[];for(const o of global.state.sortedPapers){const s=o.tags.map(e=>e.toLowerCase());!t.every(t=>s.some(e=>0<=e.indexOf(t)))||global.state.showFavorites&&!o.favorite||a.push(o)}global.state.papersList=a},searchMemoryByCode=e=>{const a=e.replace("c:","").toLowerCase().split(" ");let o=[];for(const s of global.state.sortedPapers){let t=s.codeLink||"";t=t.toLowerCase(),!a.every(e=>t.includes(e))||global.state.showFavorites&&!s.favorite||o.push(s)}global.state.papersList=o},updatePaperTagsHTML=e=>{setHTML(findEl(e,"tag-list"),global.state.papers[e].tags.map(e=>`<span class="memory-tag">${e}</span>`).join(""))},updateTagOptions=e=>{updateAllMemoryPaperTagOptions();var t=getTagsOptions(global.state.papers[e]);setHTML("popup-item-tags--"+e,t)},updatePaperTags=(t,e)=>{let a;a=e.startsWith("#")?findEl(e.replace("#","")):findEl(t,e);e=parseTags(a);updated=!1,new Set;arraysIdentical(global.state.papers[t].tags,e)||(updated=!0),global.state.papers[t].tags=e,updated&&chrome.storage.local.set({papers:global.state.papers},()=>{makeTags(),updateTagOptions(t),updatePaperTagsHTML(t);for(const e of Array.from(findEl(t,"tag-list").querySelectorAll(".memory-tag")))addListener(e,"click",handleTagClick)})},makeTags=()=>{let e=new Set;for(const t of global.state.sortedPapers)for(const a of t.tags)e.add(a);global.state.paperTags=Array.from(e),global.state.paperTags.sort()},displayMemoryTable=()=>{var e=Date.now(),t=findEl("memory-table"),a=(setHTML(t,""),{edit:"Edit paper details",copyMd:"Copy Markdown-formatted link",copyBibtext:"Copy Bibtex citation",visits:"Number of times you have opened this paper",openLocal:"Open downloaded pdf"});let o=[];for(const s of global.state.papersList)try{o.push(getMemoryItemHTML(s,a))}catch(e){log("displayMemoryTable error:"),log(e),log(s)}setHTML(t,o.join("")),addEventToClass(".back-to-focus","click",handleBackToFocus),addEventToClass(".memory-delete","click",handleDeleteItem),addEventToClass(".memory-item-link","click",handleOpenItemLink),addEventToClass(".memory-code-link","click",handleOpenItemCodeLink),addEventToClass(".memory-item-md","click",handleCopyMarkdownLink),addEventToClass(".memory-item-bibtex","click",handleCopyBibtex),addEventToClass(".memory-item-copy-link","click",handleCopyPDFLink),addEventToClass(".memory-item-openLocal","click",handleMemoryOpenLocal),addEventToClass(".memory-item-favorite","click",handleAddItemToFavorites),addEventToClass(".cancel-note-form","click",handleCancelPaperEdit),addEventToClass(".memory-item-edit","click",handleTogglePaperEdit),addEventToClass(".memory-tag","click",handleTagClick),setFormChangeListener(void 0,!1),addEventToClass(".form-note-textarea","focus",handleTextareaFocus);t=Date.now();info("Display duration (s): "+(t-e)/1e3)},makeMemoryHTML=async()=>{setPlaceholder("memory-search",`Search ${global.state.papersList.length} entries ...`),displayMemoryTable();let e=300;global.state.papersList.length<20?e=0:global.state.papersList.length<100&&(e=150),addListener("memory-search","keypress",delay(handleMemorySearchKeyPress(),e)),addListener("memory-search","clear-search",handleMemorySearchKeyPress(!0)),addListener("memory-search","keyup",handleMemorySearchKeyUp),addListener("cancel-modal-button","click",handleCancelModalClick),addListener("confirm-modal-button","click",handleConfirmDeleteModalClick),addListener("filter-favorites","click",handleFilterFavorites),addListener("memory-select","change",handleMemorySelectChange),addListener("memory-sort-arrow","click",handleMemorySortArrow)},openMemory=()=>{global.state.menuIsOpen&&closeMenu(),global.state.memoryIsOpen=!0,hideId("memory-switch-open"),showId("memory-switch-close"),hideId("menu-switch"),dispatch("memory-switch","blur"),slideDown("memory-container",200,()=>{setTimeout(()=>{dispatch("memory-search","focus")},100)}),setTimeout(()=>{addListener("memory-search-clear-icon","click",handleClearSearch),val("memory-select","lastOpenDate"),setMemorySortArrow("down")},200)},closeMemory=()=>{dispatch("memory-switch","blur"),hideId("memory-switch-close"),showId("memory-switch-open"),slideUp("memory-container",200,()=>{val("memory-search",""),dispatch("memory-search","clear-search"),global.state.memoryIsOpen=!1,global.state.showFavorites&&dispatch("filter-favorites","click"),showId("menu-switch","flex")})},closeMenu=()=>{slideUp("menu-container",300),setHTML("menu-switch",tablerSvg("settings","menu-switch-svg",["tabler-icon","menu-svg"])),dispatch("menu-switch","blur"),global.state.menuIsOpen=!1},openMenu=()=>{slideDown("menu-container",300),dispatch("menu-switch","blur"),setHTML("menu-switch",tablerSvg("circle-x","close-menu-btn",["tabler-icon","menu-svg"])),global.state.menuIsOpen=!0},getAndTrackPopupMenuChecks=(e,t)=>{let a={};for(const o of t){a[o]=e.hasOwnProperty(o)?e[o]:!(0<=global.menuCheckDefaultFalse.indexOf(o));const s=findEl(o);s&&(s.checked=a[o])}chrome.storage.local.set(a);for(const r of t)addListener(r,"change",handleMenuCheckChange)},showPopupModal=e=>{document.querySelectorAll(".popup-modal-content").forEach(hideId),showId(`modal-${e}-content`,"contents"),style("popup-modal-wrapper","display","flex")},setStandardPopupClicks=()=>{addListener("helpGithubLink","click",()=>{chrome.tabs.create({url:"https://github.com/vict0rsch/PaperMemory"})}),addListener("whats-new-container","click",()=>{chrome.storage.local.get("whatsnew",({whatsnew:e})=>{var t=chrome.runtime.getManifest().version;(e=void 0===e?{}:e).hasOwnProperty(t)||hideId("whats-new-marker"),chrome.storage.local.set({whatsnew:{...e,[t]:!0}}),showPopupModal("whatsnew")})}),addListener("keyboardShortcuts","click",()=>{showPopupModal("keyboard")}),addListener("keyboardShortcutsMenu","click",()=>{showPopupModal("keyboard")}),addListener("close-popup-modal","click",()=>{style("popup-modal-wrapper","display","none")}),addListener(window,"click",e=>{e.target===findEl("popup-modal-wrapper")&&style("popup-modal-wrapper","display","none")}),addListener("menu-switch","click",()=>{(global.state.menuIsOpen?closeMenu:openMenu)()}),addListener("memory-switch","click",handleMemorySwitchClick)},popupMain=async(a,e,t=!1)=>{console.log(navigator.userAgent),"PuppeteerAgent"===navigator.userAgent&&(info("Is puppet"),style(document.body,"min-width","500px"),style(document.body,"max-width","500px"),style(document.body,"width","500px"),style("popup-modal-wrapper","min-width","500px"),style("popup-modal-wrapper","max-width","500px"),style("popup-modal-wrapper","width","500px")),addListener(document,"keydown",handlePopupKeydown),chrome.storage.local.get("whatsnew",({whatsnew:e})=>{var t=chrome.runtime.getManifest().version;e&&e.hasOwnProperty(t)||showId("whats-new-marker")}),t?(hideId("memory-switch"),showId("memory-spinner"),await initState(),hideId("memory-spinner"),showId("memory-switch"),makeMemoryHTML()):setStandardPopupClicks();const o=await getStorage(global.menuStorageKeys)??{};if(getAndTrackPopupMenuChecks(o,global.menuCheckNames),addListener("advanced-configuration","click",()=>{chrome.runtime.openOptionsPage()}),addListener("full-memory","click",()=>{chrome.tabs.create({url:chrome.extension.getURL("src/fullMemory/fullMemory.html")})}),Object.values(e).some(e=>e)){setTimeout(()=>{document.body.style.height="auto",document.body.style.minHeight="450px"},0),showId("isArxiv","flex");const s=await parseIdFromUrl(a);if(!(global.state.currentId=s)||!global.state.papers.hasOwnProperty(s))return log("Unknown id "+s),await updatePopupPaperNoMemory(a),void(o.checkDirectOpen&&!o.checkNoAuto&&dispatch("memory-switch","click"));const r=global.state.papers[s];t=r.id.replaceAll(".","\\.");setTextId("popup-paper-title",r.title.replaceAll("\n","")),setTextId("popup-authors",cutAuthors(r.author,350)),r.codeLink&&(showId("popup-code-link"),setTextId("popup-code-link",r.codeLink.replace(/^https?:\/\//,""))),log("Popup paper:",r),setHTML("popup-memory-edit",getPopupEditFormHTML(r)),setHTML("popup-copy-icons",getPopupPaperIconsHTML(r,a,e)),findEl("checkFavorite--"+s).checked=r.favorite,$("#popup-item-tags--"+t).select2({...global.select2Options,width:"87%"}),addListener("popup-form-note-textarea--"+s,"focus",()=>{textareaFocusEnd(this)}),setFormChangeListener(s,!0),addListener("popup-delete-paper","click",handlePopupDeletePaper(s)),addListener("popup-memory-item-scirate--"+s,"click",()=>{var e=r.id.split("-").last();chrome.tabs.update({url:"https://scirate.com/arxiv/"+e}),window.close()}),addListener("popup-memory-item-link--"+s,"click",()=>{var e=paperToPDF(r),t=paperToAbs(r);chrome.tabs.update({url:isPdfUrl(a)?t:e}),window.close()}),addListener("popup-code-link","click",()=>{var e=findEl("popup-code-link").textContent;e&&focusExistingOrCreateNewCodeTab(e)}),addListener("popup-memory-item-copy-link--"+s,"click",()=>{var e=(o.checkPreferPdf?paperToPDF:paperToAbs)(r),t=o.checkPreferPdf?"PDF":"Abstract";copyAndConfirmMemoryItem(s,e,t+" link copied!",!0)}),addListener("popup-memory-item-md--"+s,"click",()=>{var e=(o.checkPreferPdf?paperToPDF:paperToAbs)(r),t=o.checkPreferPdf?"PDF":"Abstract",e=`[${r.title}](${e})`;copyAndConfirmMemoryItem(s,e,`Markdown link to ${t} copied!`,!0)}),addListener("popup-memory-item-bibtex--"+s,"click",()=>{var e=bibtexToString(global.state.papers[s].bibtex);copyAndConfirmMemoryItem(s,e,"Bibtex citation copied!",!0)}),addListener("popup-memory-item-openLocal--"+s,"click",async()=>{var e=await findLocalFile(r);e?chrome.downloads.open(e.id):chrome.tabs.create({url:r.pdfLink})}),addListener("popup-memory-item-download--"+s,"click",async()=>{let e=stateTitleFunction(r);global.state.menu.checkStore&&(e="PaperMemoryStore/"+e,0===(await getStoredFiles()).length&&chrome.downloads.download({url:URL.createObjectURL(new Blob([global.storeReadme])),filename:"PaperMemoryStore/IMPORTANT_README.txt",saveAs:!1})),log({title:e}),e.endsWith(".pdf")||(e+=".pdf"),e=e.replaceAll(":"," "),chrome.downloads.download({url:paperToPDF(r),filename:e})})}else o.checkDirectOpen&&dispatch("memory-switch","click")},query={active:!0,lastFocusedWindow:!0};window.location.href.includes("popup")&&chrome.tabs.query(query,async e=>{var e=e[0].url,t=(await initState(),await isPaper(e));Object.values(t).some(e=>e)||showId("notArxiv"),hideId("memory-spinner"),showId("memory-switch"),makeMemoryHTML(),popupMain(e,t),-1<navigator.userAgent.search("Firefox")&&hideId("overwrite-container")});