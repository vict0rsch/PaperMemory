    ...
    `;e=-1<navigator.userAgent.search("Firefox")||global.state.prefs.checkNoAuto;e&&(a+=`
        ...
        >
            ...
            <line x1="8" y1="15" x2="12" y2="19" />
                ...
                </div>`),showId("website-parsing-root"),addListener("website-trigger-btn","click",async()=>{hideId("website-trigger-btn"),showId("website-loader-container"),hideId("website-parsing-error");let e;try{e=await addOrUpdatePaper({tab:a,url:a.url,store:!1})}catch(e){console.log("error: ",e),hideId("website-loader-container"),showId("website-parsing-error"),setHTML("website-parsing-error",`<h3>Error</h3><div>${e}</div>`),setTimeout(()=>{hideId("website-loader-container"),hideId("website-parsing-error"),showId("website-trigger-btn")},3e3)}e?.paper&&editManualWebsite(e.paper,o)}))},query={active:!0,lastFocusedWindow:!0};window.location.href.includes("popup")&&chrome.tabs.query(query,async e=>{chrome.runtime.connect({name:"PaperMemoryPopupSync"});var t=e[0].url;let a,r;r=new Promise(t=>{a=new Promise(e=>{initSyncAndState({stateIsReady:e,remoteIsReady:t})})}),await a;var o=await isPaper(t);Object.values(o).some(e=>e)||showId("notArxiv"),hideId("memory-spinner"),showId("memory-switch"),makeMemoryHTML(),popupMain(t,o,!1,e[0]),-1<navigator.userAgent.search("Firefox")&&hideId("overwrite-container"),await r,global.state.currentId&&!global.state.papers[global.state.currentId]&&(global.state.currentId=null,makeMemoryHTML(),await updatePopupPaperNoMemory(t))});