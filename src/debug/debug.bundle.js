var PMDebug = (function () {
    ...
    var miniquery = /*#__PURE__*/Object.freeze({
        __proto__: null,
        addClass: addClass,
        addEventToClass: addEventToClass,
    ...
    const getDisplayId = (id) => {
        ...
        if (state$1.papers.hasOwnProperty(baseId)) {
            const paper = state$1.papers[baseId];
    ...
    var functions = /*#__PURE__*/Object.freeze({
        ...
        arxivIdFromPaperID: arxivIdFromPaperID,
        arxivIdFromURL: arxivIdFromURL,
        cleanBiorxivURL: cleanBiorxivURL,
        cleanPapers: cleanPapers,
        cleanStr: cleanStr,
        consoleHeader: consoleHeader,
        copyHyperLinkToClipboard: copyHyperLinkToClipboard,
        copyTextToClipboard: copyTextToClipboard,
        cutAuthors: cutAuthors,
        debug: debug,
        dedent: dedent,
        delay: delay,
        ...
        firstNonStopLowercase: firstNonStopLowercase,
        getBrowserName: getBrowserName,
        getDisplayId: getDisplayId,
        getPaperEdits: getPaperEdits,
        ...
        parseCVFUrl: parseCVFUrl,
        parseTags: parseTags,
        parseUrl: parseUrl$1,
        sendMessageToBackground: sendMessageToBackground,
        shouldWarn: shouldWarn,
        silentPromiseTimeout: silentPromiseTimeout,
        spaceCamelCase: spaceCamelCase,
        stringifyError: stringifyError,
        tablerSvg: tablerSvg,
        textareaFocusEnd: textareaFocusEnd,
        toSingleSpace: toSingleSpace,
        urlToWebsiteId: urlToWebsiteId,
        warn: warn
    });
...
`;
    ...
    var config = /*#__PURE__*/Object.freeze({
        __proto__: null,
        consolHeaderStyle: consolHeaderStyle,
        descendingSortKeys: descendingSortKeys,
        englishStopWords: englishStopWords,
        journalAbbreviations: journalAbbreviations,
        knownPaperPages: knownPaperPages,
        overrideDBLPVenues: overrideDBLPVenues,
        overrideORConfs: overrideORConfs,
        overridePMLRConfs: overridePMLRConfs,
        prefsCheckDefaultFalse: prefsCheckDefaultFalse,
        prefsCheckNames: prefsCheckNames,
        prefsStorageKeys: prefsStorageKeys,
        preprintSources: preprintSources,
        select2Options: select2Options,
        sourceExtras: sourceExtras,
        state: state$1,
        storeReadme: storeReadme,
        svgActionsHoverTitles: svgActionsHoverTitles
    });
    ...
    var bibtexParser = /*#__PURE__*/Object.freeze({
        __proto__: null,
        BibtexParser: BibtexParser,
        bibtexToObject: bibtexToObject,
        bibtexToString: bibtexToString,
        extractBibtexValue: extractBibtexValue,
        safeRemoveSurroundingBraces: safeRemoveSurroundingBraces
    });
    ...
    const parseIdFromUrl$1 = async (url, tab = null) => {
        ...
        const hashedId = state$1.urlHashToId[hashedUrl];
        if (hashedId) {
            return hashedId;
        }

        const is = await isPaper(url, true);
        const papers = Object.values(cleanPapers(state$1.papers));

        if (is.arxiv) {
            let arxivId = arxivIdFromURL(url);
            idForUrl = `Arxiv-${arxivId}`;

            const existingIds = Object.values(state$1.titleHashToIds).find((ids) =>
            ...
            const [journal, type] = parseUrl$1(url.split("#")[0])
                .pathname.split("/")
                .slice(1, 3);
    ...
    const findLocalFile = async (paperOrUrl) => {
        ...
        if (typeof paperOrUrl === "string") {
            ...
            if (state$1.papers.hasOwnProperty(id)) {
                paper = state$1.papers[id];
            } else {
    ...
    const isKnownLocalFile = (url) => {
        ...
        const storedPaths = Object.entries(state$1.files).filter(
        ...
        const filename = decodeURIComponent(url.split("/").last())
            .toLowerCase()
            .replace(/\W/g, "");
        const titles = Object.values(cleanPapers(state$1.papers))
            .map((p) => {
    ...
    var files = /*#__PURE__*/Object.freeze({
        ...
        isKnownLocalFile: isKnownLocalFile,
        matchPapersToFiles: matchPapersToFiles
    });
    ...
    const isPaper = async (url, noStored = false) => {
        ...
        is.parsedWebsite = state$1.papers[`Website_${urlToWebsiteId(url)}`] ?? false;
        return is;
    };
    ...
    const addOrUpdatePaper = async ({
        ...
        // Extract id from url
        state$1.papers = (await getStorage("papers")) ?? {};
        const id = await parseIdFromUrl$1(url, tab);
        const paperExists = state$1.papers.hasOwnProperty(id);
        prefs &&
            prefs.checkFeedback &&
            typeof feedback !== "undefined" &&
            feedback({ loading: true });

        if (id && paperExists && state$1.papers[id].author.toLowerCase() !== "anonymous") {
            // Update paper if it exists
            paper = updatePaperVisits(state$1.papers[id]);
            isNew = false;
        } else {
            ...
            state$1.titleHashToIds = makeTitleHashToIdList(state$1.papers);
            const existingId = findFuzzyPaperMatch(state$1.titleHashToIds, newPaper);
            if (existingId && store) {
                // Update paper as already it exists
                let existingPaper = state$1.papers[existingId];
                ...
                if (existingPaper.venue) {
                    ...
                    await deletePaperInStorage(existingPaper.id, state$1.papers);
                    existingPaper = mergePapers({
                        newPaper: newPaper,
                        oldPaper: existingPaper,
                        incrementCount: false,
        ...
        if (isNew && state$1.papers.hasOwnProperty(paper.id)) {
            warn("Paper has been created by another page: merging papers.");
            paper = mergePapers({
                newPaper: state$1.papers[paper.id],
                oldPaper: paper,
                incrementCount: true,
            });
            isNew = false;
        ...
        if (store && !state$1.deleted[paper.id]) state$1.papers[paper.id] = paper;

        chrome.storage.local.set({ papers: state$1.papers }, async () => {
            ...
            if (!paper.note || !paper.venue || paper.author.toLowerCase() === "anonymous") {
                ...
                if (
                    isNew &&
                    state$1.papers.hasOwnProperty(paper.id) &&
                    state$1.papers[paper.id].count > 1
                ) {
                    warn("Paper has been created by another page: merging papers.");
                    paper = mergePapers({
                        newPaper: state$1.papers[paper.id],
                ...
                await new Promise((resolve) =>
                    chrome.storage.local.set({ papers: state$1.papers }, resolve)
                );
            }
    ...
    const addPaperToTitleHashToId = (paper) => {
        ...
        if (!state$1.titleHashToIds.hasOwnProperty(hashedTitle)) {
            state$1.titleHashToIds[hashedTitle] = [];
        }
        if (!state$1.titleHashToIds[hashedTitle].includes(id)) {
            state$1.titleHashToIds[hashedTitle].push(id);
        }
    };

    var paper = /*#__PURE__*/Object.freeze({
        __proto__: null,
        addOrUpdatePaper: addOrUpdatePaper,
        addPaperToTitleHashToId: addPaperToTitleHashToId,
        findFuzzyPaperMatch: findFuzzyPaperMatch,
        isPaper: isPaper,
        makeMdLink: makeMdLink,
        mergePapers: mergePapers,
        paperToAbs: paperToAbs,
        paperToPDF: paperToPDF,
        updatePaperVisits: updatePaperVisits
    });
    ...
    const initState = async ({ papers, isContentScript, print = true } = {}) => {
        ...
        state$1.dataVersion = getManifestDataVersion();
        ...
        const migration = await migrateData(papers, state$1.dataVersion);
        print && log("Time to migrate data (s): " + duration(times));
        times.unshift(Date.now());

        papers = migration.papers;
        state$1.papers = papers;

        state$1.prefs = await getPrefs();
        print && log("Time to retrieve user preferences (s): " + duration(times));
        times.unshift(Date.now());

        state$1.ignoreSources = (await getStorage("ignoreSources")) ?? {};
        print && log("Time to retrieve sources to ignore (s): " + duration(times));
        times.unshift(Date.now());

        state$1.urlHashToId = (await getStorage("urlHashToId")) ?? {};
        print && log("Time to retrieve sources to urlHashToId (s): " + duration(times));
        times.unshift(Date.now());

        state$1.titleHashToIds = makeTitleHashToIdList(papers);
        print && log("Time to hash titles (s): " + duration(times));
        times.unshift(Date.now());

        if (!isContentScript) {
            state$1.files = await matchAllFilesToPapers();
            print && log("Time to match all local files (s): " + duration(times));
            times.unshift(Date.now());

            state$1.papersList = Object.values(cleanPapers(papers));
            state$1.sortKey = "lastOpenDate";
            state$1.papersReady = true;
        ...
        (async () => {
            ...
            state$1.cellJournalData = cellData;
        })();
    };
     ...
     */
    const sortMemory = () => {
        state$1.sortedPapers = Object.values(cleanPapers(state$1.papers));
        state$1.sortedPapers.sort(
            orderPapers(descendingSortKeys.indexOf(state$1.sortKey) >= 0)
        );
        state$1.papersList.sort(orderPapers(descendingSortKeys.indexOf(state$1.sortKey) >= 0));
    };
    ...
    const orderPapers = (descending) => (paper1, paper2) => {
        let val1 = paper1[state$1.sortKey];
        let val2 = paper2[state$1.sortKey];
    ...
    const makeTags = () => {
        let tags = new Set();
        for (const p of state$1.sortedPapers) {
            for (const t of p.tags) {
                tags.add(t);
            }
        }
        state$1.paperTags = [...tags];
        state$1.paperTags.sort();
    };
    ...
    const stateTitleFunction = (paperOrId) => {
        ...
        if (typeof paperOrId === "string") {
            // paperOrId is an ID
            paper = state$1.papers[paperOrId];
            if (typeof paper === "undefined") {
                // no such paper
        ...
        const name = state$1.titleFunction(paper);
    ...
    var state = /*#__PURE__*/Object.freeze({
        __proto__: null,
        downloadPaperPdf: downloadPaperPdf,
        duration: duration,
        getExamplePaper: getExamplePaper,
        getTagsOptions: getTagsOptions,
        initState: initState,
        makeTags: makeTags,
        matchAllFilesToPapers: matchAllFilesToPapers,
        orderPapers: orderPapers,
        readJournalAbbreviations: readJournalAbbreviations,
        sortMemory: sortMemory,
        stateTitleFunction: stateTitleFunction
    });
    ...
    const getMemoryItemHTML = (paper) => {
        ...
        state$1.prefs.checkPreferPdf ? "PDF" : "abstract"
    }`;
    ...
    `;
        ...
        const openLocalDiv = state$1.files.hasOwnProperty(paper.id)
            ? /*html*/ `
            <div
        ...
        let scirate = "";
        if (state$1.prefs.checkScirate && paper.source === "arxiv") {
            scirate = /*html*/ `
        <div
        ...
        let alphaxiv = "";
        if (state$1.prefs.checkAlphaxiv && paper.source === "arxiv") {
            alphaxiv = /*html*/ `
        <div
        ...
        let ar5iv = "";
        if (state$1.prefs.checkAr5iv && paper.source === "arxiv") {
            ar5iv = /*html*/ `
        <div
        ...
        let huggingface = "";
        if (state$1.prefs.checkHuggingface && paper.source === "arxiv") {
            huggingface = /*html*/ `
        <div
...
${note}</textarea
    ...
    const getPopupPaperIconsHTML = (paper, currentUrl, is) => {
        ...
        let huggingface = "";
        if (
            state$1.prefs.checkHuggingface &&
            paper.source === "arxiv" &&
        ...
        const download =
            state$1.prefs.checkStore &&
            (is.localFile || is.stored || state$1.files.hasOwnProperty(paper.id))
                ? /*html*/ `
        <div
            tabindex="0"
            class="memory-item-svg-div"
    ...
    var templates = /*#__PURE__*/Object.freeze({
        __proto__: null,
        getMemoryItemHTML: getMemoryItemHTML,
        getPaperInfoTable: getPaperInfoTable,
        getPopupEditFormHTML: getPopupEditFormHTML,
        getPopupPaperIconsHTML: getPopupPaperIconsHTML
    });
    ...
    const focusExistingOrCreateNewPaperTab = async (paper, fromMemoryItem) => {
        ...
        chrome.tabs.query({}, async (tabs) => {
            ...
            if (favoriteTabs.length > 0) {
                ...
                const fileTabs =
                    fromMemoryItem && state$1.files.hasOwnProperty(paper.id)
                        ? []
            ...
            if (tabToFocus) {
                ...
                // no tab was found
                const hasFile = state$1.files.hasOwnProperty(paper.id);
                if (hasFile && !fromMemoryItem) {
                    // this paper has a local file
                    chrome.downloads.open(state$1.files[paper.id].id);
                } else {
            ...
            state$1.papers[paper.id] = updatePaperVisits(state$1.papers[paper.id]);
            chrome.storage.local.set({ papers: state$1.papers });
        });
    };
    ...
    const showConfirmDeleteModal = (id) => {
        const title = state$1.papers[id].title;
    ...
    const monitorPaperEdits = (id, isPopup) => (e) => {
        ...
        const paper = state$1.papers[paperId];
        let change = false;
    ...
    const displayOnScroll = (isPopup) =>
        delay(() => {
            ...
            const height = isPopup
                ...
                : window.innerHeight;
            const currentPapers = state$1.currentMemoryPagination * state$1.memoryItemsPerPage;
            if (
                Math.abs(bottom - height) < height &&
                currentPapers < state$1.papersList.length
            ) {
                state$1.currentMemoryPagination += 1;
                displayMemoryTable(state$1.currentMemoryPagination);
            }
        }, 50);
    ...
    const handleOpenItemLink = (e) => {
        const id = eventId(e);
        focusExistingOrCreateNewPaperTab(state$1.papers[id], true);
    };

    const handleOpenItemScirate = (e) => {
        const id = eventId(e);
        const arxivId = arxivIdFromPaperID(state$1.papers[id].id);
        const scirateURL = `https://scirate.com/arxiv/${arxivId}`;
        focusExistingOrCreateNewURLTab(scirateURL);
        state$1.papers[id] = updatePaperVisits(state$1.papers[id]);
        setStorage("papers", state$1.papers);
    };
    ...
    const handleOpenItemCodeLink = async (e) => {
        const id = eventId(e);
        const url = state$1.papers[id].codeLink;
        await focusExistingOrCreateNewURLTab(url);
    };

    const handleOpenItemWebsiteURL = async (e) => {
        const id = eventId(e);
        const url = state$1.papers[id].pdfLink;
        state$1.papers[id] = updatePaperVisits(state$1.papers[id]);
        await setStorage("papers", state$1.papers);
        await focusExistingOrCreateNewURLTab(url);
    };
    ...
    const handleCopyMarkdownLink = async (e) => {
        ...
        await copyAndConfirmMemoryItem({
            ...
            context: state$1.memoryIsOpen ? "memory" : "popup",
        });
    };

    const handleCopyBibtex = async (e) => {
        const id = eventId(e);
        const bibtex = state$1.papers[id].bibtex;
        let bibobj = bibtexToObject(bibtex);
        if (!bibobj.hasOwnProperty("url")) {
            bibobj.url = paperToAbs(state$1.papers[id]);
        }
        if (!bibobj.hasOwnProperty("pdf") && state$1.papers[id].source !== "website") {
            bibobj.pdf = paperToPDF(state$1.papers[id]);
        }
        await copyAndConfirmMemoryItem({
            id,
    ...
    const handleMemoryOpenLocal = (e) => {
        const id = eventId(e);
        const file = state$1.files[id];
        const paper = state$1.papers[id];
        state$1.papers[id] = updatePaperVisits(paper);
        setStorage("papers", state$1.papers);
    ...
    const handleFilterFavorites = () => {
        const showFavorites = !state$1.showFavorites;
        state$1.showFavorites = showFavorites;
        if (showFavorites) {
            addClass(
    ...
    const handleMemorySearchKeyPress = (allowEmptySearch) => (e) => {
        ...
        if (!query) {
            if (state$1.papersList.length !== state$1.sortedPapers.length) {
    ...
    const handleConfirmDeleteModalClick = async (e) => {
        ...
        const title = state$1.papers[id].title;
        const url = state$1.papers[id].pdfLink;
        await deletePaperInStorage(id, state$1.papers);
        displayMemoryTable();
        hideId("delete-paper-modal");
        info(`Successfully deleted "${title}" (${id}) from PaperMemory`);
        if (state$1.currentId === id) {
    ...
    const handleMemorySwitchClick = () => {
        state$1.memoryIsOpen ? closeMemory() : openMemory();
    };
    ...
    const handlePopupKeydown = async (e) => {
        ...
        if (key === "Escape" && state$1.tooltipIsOpen) {
        ...
        if (!state$1.memoryIsOpen) {
            if (key === "a") {
                // a opens the arxiv memory
                state$1.papers && dispatch("memory-switch", "click");
            } else if (key === "Enter") {
        ...
        let id, paperItem;
        if (state$1.currentId && !state$1.memoryIsOpen) {
            id = state$1.currentId;
        } else {
        ...
        if (key === "Backspace") {
            ...
            // open paper
            const target =
                state$1.papers[id].source === "website"
                    ? localFindEl({ id, paperItem, memoryItemClass: "memory-website-url" })
                    : (state$1.prefs.checkEnterLocalPdf &&
                          localFindEl({
                              id,
                              paperItem,
    ...
    const handlePrefsCheckChange = async (e) => {
        ...
        if (state$1 && state$1.prefs) {
            state$1.prefs[key] = checked;
            setStorage("prefs", state$1.prefs, function () {
    ...
    const getHandleTitleTooltip = (func, delay, isPopup) => {
        return (e) => {
            const id = isPopup ? state$1.currentId : eventId(e);
            let timerId = state$1.timerIdMap.get(e.target) ?? 0;
            clearTimeout(timerId);
            timerId = setTimeout(() => func(id, isPopup), delay);
            state$1.timerIdMap.set(e.target, timerId);
        };
    };
    ...
    const handleExpandAuthors = (e) => {
        ...
        setHTML(authorsEl, cutAuthors(state$1.papers[id].author, 100000));
    };
    ...
    const addEventsToMemoryControls = () => {
        ...
        let delayTime = 300;
        if (state$1.papersList.length < 20) {
            delayTime = 0;
        } else if (state$1.papersList.length < 100) {
            delayTime = 150;
        }
    ...
    var handlers = /*#__PURE__*/Object.freeze({
        __proto__: null,
        addEventsToMemoryControls: addEventsToMemoryControls,
        addEventsToMemoryItems: addEventsToMemoryItems,
        closePopupModal: closePopupModal,
        displayOnScroll: displayOnScroll,
        focusExistingOrCreateNewPaperTab: focusExistingOrCreateNewPaperTab,
        getHandleTitleTooltip: getHandleTitleTooltip,
        handleAddItemToFavorites: handleAddItemToFavorites,
        handleBackToFocus: handleBackToFocus,
        handleCancelModalClick: handleCancelModalClick,
        handleCancelPaperEdit: handleCancelPaperEdit,
        handleClearSearch: handleClearSearch,
        handleConfirmDeleteModalClick: handleConfirmDeleteModalClick,
        handleCopyBibtex: handleCopyBibtex,
        handleCopyHyperLink: handleCopyHyperLink,
        handleCopyMarkdownLink: handleCopyMarkdownLink,
        handleCopyPDFLink: handleCopyPDFLink,
        handleDeleteItem: handleDeleteItem,
        handleExpandAuthors: handleExpandAuthors,
        handleFilterFavorites: handleFilterFavorites,
        handleHideAllTitleTooltips: handleHideAllTitleTooltips,
        handleMemoryOpenLocal: handleMemoryOpenLocal,
        handleMemorySaveEdits: handleMemorySaveEdits,
        handleMemorySearchKeyPress: handleMemorySearchKeyPress,
        handleMemorySearchKeyUp: handleMemorySearchKeyUp,
        handleMemorySelectChange: handleMemorySelectChange,
        handleMemorySortArrow: handleMemorySortArrow,
        handleMemorySwitchClick: handleMemorySwitchClick,
        handleOpenItemAlphaxiv: handleOpenItemAlphaxiv,
        handleOpenItemAr5iv: handleOpenItemAr5iv,
        handleOpenItemCodeLink: handleOpenItemCodeLink,
        handleOpenItemHuggingface: handleOpenItemHuggingface,
        handleOpenItemLink: handleOpenItemLink,
        handleOpenItemScirate: handleOpenItemScirate,
        handleOpenItemWebsiteURL: handleOpenItemWebsiteURL,
        handlePopupDeletePaper: handlePopupDeletePaper,
        handlePopupKeydown: handlePopupKeydown,
        handlePopupSaveEdits: handlePopupSaveEdits,
        handlePrefsCheckChange: handlePrefsCheckChange,
        handleTagClick: handleTagClick,
        handleTextareaFocus: handleTextareaFocus,
        handleTogglePaperEdit: handleTogglePaperEdit,
        hideAllTooltips: hideAllTooltips,
        hideTitleTooltip: hideTitleTooltip,
        monitorPaperEdits: monitorPaperEdits,
    ...
    const setStandardPopupClicks = () => {
        ...
        addListener("menu-switch", "click", () => {
            state$1.prefsIsOpen ? closeMenu() : openMenu();
        });
    ...
    const popupMain = async (url, is, manualTrigger = false, tab = null) => {
        ...
        if (Object.values(is).some((i) => i)) {
            ...
            state$1.currentId = id;

            if (!id || !state$1.papers.hasOwnProperty(id)) {
            ...
            addListener(`popup-memory-item-bibtex--${id}`, "click", async () => {
                let bibtex = state$1.papers[id].bibtex;
                let bibobj = bibtexToObject(bibtex);
                if (!bibobj.hasOwnProperty("url")) {
                    bibobj.url = paperToAbs(state$1.papers[id]);
                }
            ...
            addListener(`popup-memory-item-openLocal--${id}`, "click", async () => {
                const file = (await findLocalFile(paper)) || state$1.files[paper.id];
                if (file) {
                    chrome.downloads.open(file.id);
                } else {
            ...
            // ------------------------------------
            const allowWebsiteParsing = tab && state$1.prefs.checkWebsiteParsing;
    ...
    if (
        ...
        chrome.tabs.query(query, async (tabs) => {
            ...
            if (state$1.currentId && !state$1.papers[state$1.currentId]) {
                state$1.currentId = null;
                makeMemoryHTML();
                await updatePopupPaperNoMemory(url);
            }
        });
    ...
    const updateAllMemoryPaperTagOptions = () => {
        for (const id in state$1.papers) {
            if (state$1.papers.hasOwnProperty(id) && id !== "__dataVersion") {
                const paper = state$1.papers[id];
    ...
    `;
        ...
        const allowManualParsing = isFirefox || state$1.prefs.checkNoAuto;
        let ff_warning = "";
        if (isFirefox) {
            ff_warning = /* html */ `
            <div id="ff-warning">
    ...
    const saveCodeLink = (id, codeLink) => {
        ...
        state$1.papers[id].codeLink = codeLink;
        chrome.storage.local.set({ papers: state$1.papers }, () => {
    ...
    const saveFavoriteItem = (id, favorite) => {
        state$1.papers[id].favorite = favorite;
        state$1.papers[id].favoriteDate = new Date().toJSON();
        chrome.storage.local.set({ papers: state$1.papers }, () => {
            if (favorite) {
            ...
            if (state$1.sortKey === "favoriteDate") {
                if (!favorite) {
                    sortMemory();
                    displayMemoryTable();
                }
                const n = state$1.sortedPapers.filter((p) => p.favorite).length;
     ...
     */
    const reverseMemory = () => {
        state$1.sortedPapers.reverse();
        state$1.papersList.reverse();
    };
    ...
    const searchMemory = (letters) => {
        ...
        for (const paper of state$1.sortedPapers) {
            ...
            if (words.every((w) => contents.some((c) => c.includes(w)))) {
                if (!state$1.showFavorites || paper.favorite) {
                    papersList.push(paper);
                }
            }
        }
        state$1.papersList = papersList;
    };
    ...
    const updateTagOptions = (id) => {
        ...
        const tagOptions = getTagsOptions(state$1.papers[id]);
    ...
    const updatePaperTags = (id, elementId) => {
        ...
        let updated = false;
        if (!arraysIdentical(state$1.papers[id].tags, tags)) updated = true;
        state$1.papers[id].tags = tags;
    ...
    const displayMemoryTable = (pagination = 0) => {
        ...
        if (pagination === 0) {
            setHTML(memoryTable, "");
            state$1.currentMemoryPagination = 0;
        }

        // Add relevant sorted papers (papersList may be smaller than sortedPapers
        // depending on the search query)
        let table = [];
        for (const paper of state$1.papersList.slice(
            pagination * state$1.memoryItemsPerPage,
            (pagination + 1) * state$1.memoryItemsPerPage
        )) {
            try {
    ...
    const openMemory = () => {
        state$1.prefsIsOpen && closeMenu();
        state$1.memoryIsOpen = true;
        // hide menu button
    ...
    const closeMemory = () => {
        ...
        slideUp("memory-container", 200, () => {
            val("memory-search", "");
            dispatch("memory-search", "clear-search");
            state$1.memoryIsOpen = false;
            if (state$1.showFavorites) {
    ...
    var memory = /*#__PURE__*/Object.freeze({
        ...
        copyAndConfirmMemoryItem: copyAndConfirmMemoryItem,
        displayMemoryTable: displayMemoryTable,
        focusExistingOrCreateNewURLTab: focusExistingOrCreateNewURLTab,
        makeMemoryHTML: makeMemoryHTML,
        openMemory: openMemory,
        reverseMemory: reverseMemory,
        sampleAsciiArt: sampleAsciiArt,
        saveCodeLink: saveCodeLink,
        ...
        updatePaperTags: updatePaperTags,
        updatePaperTagsHTML: updatePaperTagsHTML,
        updatePopupPaperNoMemory: updatePopupPaperNoMemory,
    ...
    // pkg/dist-src/defaults.js
    var userAgent = `octokit-endpoint.js/${VERSION$1} ${getUserAgent()}`;
    var DEFAULTS = {
      method: "GET",
    ...
    const initSyncAndState = async ({
        ...
        if (!state$1.dataVersion || forceInit) {
    ...
    var sync = /*#__PURE__*/Object.freeze({
        __proto__: null,
        createGistWithFile: createGistWithFile,
        deleteGist: deleteGist,
        errorSyncLoader: errorSyncLoader,
        getDataForGistFile: getDataForGistFile,
        getGist: getGist,
        getIdentifier: getIdentifier,
        getPat: getPat,
        initSyncAndState: initSyncAndState,
        measureCacheTime: measureCacheTime,
        pullFromRemote: pullFromRemote,
        pushToRemote: pushToRemote,
        shouldSync: shouldSync,
        sleep: sleep,
        startSyncLoader: startSyncLoader,
        successSyncLoader: successSyncLoader,
        updateGistFile: updateGistFile
    });
    ...
    const makeNeuripsPaper = async (url) => {
        ...
        if (citeUrl) {
            bibtex = await fetchText(`https://${parseUrl$1(url).host}${citeUrl}`);
    ...
    const makeIOPPaper = async (url) => {
        ...
        const citeUrl = `https://${parseUrl$1(url).host}${bibtexPath}`;
        const bibtex = await fetchText(citeUrl);
        const data = bibtexToObject(bibtex);
        const author = data.author.replaceAll("\n", "").trim();
        const title = data.title.trim();
    ...
    const makeIEEEPaper = async (url) => {
        ...
        const pdfLink = `${parseUrl$1(url).origin}${metadata.pdfUrl}`;
    ...
    const makeAPSPaper = async (url) => {
        url = url.split("#")[0];
        const [journal, type] = parseUrl$1(url).pathname.split("/").slice(1, 3);
        const doi = url.split(`/${journal}/${type}/`).last();
    ...
    const makeWebsitePaper = async (tab) => {
        ...
        const author =
            og.site_name || parseUrl$1(url).hostname.replace("www.", "").capitalize();
    ...
    const findCellPii = async (url) => {
        ...
        if (isPdf || isPdfExtended) {
            while (!state$1.cellJournalData) {
                console.log("Waiting for cell journal data...");
                await sleep(5);
            }
            const cellData = state$1.cellJournalData;
    ...
    const deletePaperInStorage = async (id, papers) => {
        ...
        if (papers.hasOwnProperty(id)) {
            ...
            deleted = delete state$1.titleHashToIds[miniHash(papers[id].title)];
            deleted = deleted && delete papers[id];
            delete state$1.papers[id];
        }
        if (deleted) {
            await setStorage("papers", papers);
            state$1.papersList = Object.values(cleanPapers(state$1.papers));
            sortMemory();
    ...
    const updateDuplicatedUrls = (url, id, remove = false) => {
        if (!remove) {
            state$1.urlHashToId[miniHash(url)] = id;
            setStorage("urlHashToId", state$1.urlHashToId);
        } else {
            let hashedUrls;
            if (!url) {
                hashedUrls = Object.keys(state$1.urlHashToId).filter(
                    (k) => state$1.urlHashToId[k] === id
                );
            } else {
            ...
            if (hashedUrls && hashedUrls.length) {
                for (const hashedUrl of hashedUrls) {
                    ...
                    delete state$1.urlHashToId[hashedUrl];
                }
                setStorage("urlHashToId", state$1.urlHashToId);
            }
        }
    ...
    var data = /*#__PURE__*/Object.freeze({
        ...
        deletePaperInStorage: deletePaperInStorage,
        getDefaultKeyboardAction: getDefaultKeyboardAction,
        getManifestDataVersion: getManifestDataVersion,
        getPrefs: getPrefs,
        getStorage: getStorage,
        getTheme: getTheme,
        logStorage: logStorage,
        makeTitleHashToIdList: makeTitleHashToIdList,
        makeVenue: makeVenue,
        migrateData: migrateData,
        prepareOverwriteData: prepareOverwriteData,
        setDefaultKeyboardAction: setDefaultKeyboardAction,
        setStorage: setStorage,
        updateDuplicatedUrls: updateDuplicatedUrls,
        validatePaper: validatePaper,
        versionToSemantic: versionToSemantic,
