...
`+`url={${e}},
`)+`year={${s}}
`+"}",i=bibtexToString(i)}var r=e.replace("/hash/","/file/").replace("-Abstract.html","-Paper.pdf"),e=`NeurIPS-${s}_`+t.slice(0,8),p="NeurIPS",c=`Accepted @ ${p} (${s})`;return{author:o,bibtex:i,id:e,key:l,note:c,pdfLink:r,title:n,venue:p,year:s}},makeCVFPaper=async e=>{const t=await fetchCvfHTML(e),a=(new DOMParser).parseFromString(t.replaceAll("\n",""),"text/html");var r=a.getElementById("papertitle").innerText.trim(),i=a.querySelector("#authors i").innerText.split(",").map(e=>e.trim()).join(" and "),{year:o,id:n,conf:s}=parseCVFUrl(e);let l="";if(e.endsWith(".pdf"))l=e;else{let e=Array.from(a.getElementsByTagName("a")).filter(e=>"pdf"===e.innerText)[0].getAttribute("href");(e=e.startsWith("../")?e.replaceAll("../",""):e).startsWith("/")||(e="/"+e),l="http://openaccess.thecvf.com"+e}e=s,s=`Accepted @ ${e} (${o})`;const p=bibtexToString(a.querySelector(".bibref").innerText);var c=p.split("{")[1].split(",")[0];return{author:i,bibtex:p,id:n,key:c,note:s,pdfLink:l,title:r,venue:e,year:o}},makeOpenReviewBibTex=(e,t)=>{var a=e.content.title,r=e.content.authors.join(" and "),i=e.cdate?new Date(e.cdate).getFullYear():"0000",e=(e.cdate||log("makeOpenReviewBibTex: no cdate found in",e),e.content.authors[0].split(" ").last()),o="";return o+`@inproceedings{${e=(e+=i)+firstNonStopLowercase(a)},
`+`title={${a}},
`+`author={${r}},
`+`year={${i}},
`+`url={${t}},
`+"}"},makeOpenReviewPaper=async e=>{var t=await fetchOpenReviewNoteJSON(e),a=await fetchOpenReviewForumJSON(e),t=t.notes[0],a=a.notes,r=t.content.title,i=t.content.authors.join(" and "),o=bibtexToString(t.content._bibtex||makeOpenReviewBibTex(t,e)),n=bibtexToObject(o),s=n.citationKey,n=n.year;let l;l=t.pdf?"https://openreview.net/pdf?id="+t.id:(t.html||e).replace("/forum?id=","/pdf?id=");const p=t.invitation.split("/");let c=p[0].split(".")[0],d=p.slice(1).join("/").split("-")[0].replaceAll("/"," ").replace(" Conference",""),u=c,h=!1;global.overrideORConfs.hasOwnProperty(c)&&(u=global.overrideORConfs[c],h=!0),h&&(d=d.replace(u,""),c=u);e=(c+" "+d).replace(/ \d\d\d\d/g,"").replace(/\s\s+/g," "),t=`OR-${c}-${n}_`+t.id;let f,m,g,y="";return(f=a.filter(e=>e&&e.content&&-1<["Final Decision","Paper Decision","Acceptance Decision"].indexOf(e.content.title)))&&0<f.length&&(m=f[0].content.decision.split(" ").map((e,t)=>0===t?e+"ed":e).join(" "),g=m+` @ ${e} (${n})`,m.toLowerCase().indexOf("rejected")<0&&(y=e)),{author:i,bibtex:o,id:t,key:s,note:g="Anonymous"===i?`Under review @ ${e} (${n}) (${(new Date).toLocaleDateString()})`:g,pdfLink:l,title:r,venue:y,year:n}},makeBioRxivPaper=async e=>{var t=e.replace(".full.pdf",""),a="https://api.biorxiv.org//details/biorxiv/"+e.split("/").slice(-2).join("/").replace(".full.pdf","").split("v")[0];const r=await fetch(a).then(e=>e.json());if("ok"!==r.messages[0].status)throw new Error(a+" returned "+r.messages[0].status);const i=r.collection.last(),o=await fetchText(t),n=(new DOMParser).parseFromString(o.replaceAll("\n",""),"text/html");a=n.querySelector(".bibtext a").getAttribute("href");const s=bibtexToString(await(await fetch(a)).text());var t=extractAuthor(s),a=await parseIdFromUrl(e),l=s.split("\n")[0].split("{")[1].replace(",","").trim(),e=cleanBiorxivURL(e)+".full.pdf",p=i.title,c=i.date.split("-")[0];return{author:t,bibtex:s,id:a,key:l,note:"",pdfLink:e,title:p,venue:"",year:c}},makePMLRPaper=async e=>{var t=e.split("/").last().split(".")[0],a=await parseIdFromUrl(e);const r=e.includes(".html")?e:e.split("/").slice(0,-2).join("/")+`/${t}.html`;e=r.replace(".html","")+`/${t}.pdf`;const i=await fetchDom(r);i.getElementById("button-bibtex1").getAttribute("onclick").match(/https.+\.bib/)[0];const o=i.getElementById("bibtex").innerText.replaceAll("\t"," ").replaceAll(/\s\s+/g," ");let n=o;for(const u of o.match(/,\ ?\w+ ?= ?{/g))n=n.replace(u,u.replace(", ",",\n    ").replace(" = ","="));n.endsWith("}}")&&(n=n.slice(0,-2)+"}\n}"),n=bibtexToString(n);var s=extractAuthor(n),l=i.getElementsByTagName("h1")[0].innerText,p=extractBibtexValue(n,"year");let c=extractBibtexValue(n,"booktitle").replaceAll("Proceedings of the",""),d=c;note=`Accepted @ ${d} (${p})`;for(const h in global.overridePMLRConfs)if(c.includes(h)){d=global.overridePMLRConfs[h],c=d+" "+p,note="Accepted @ "+c;break}return{author:s,bibtex:n,id:a,key:t,note:note,pdfLink:e,title:l,venue:d,year:p}},findACLValue=(e,t)=>{return Array.from(e.querySelectorAll("dt")).filter(e=>e.innerText.includes(t))[0].nextElementSibling.innerText},makeACLPaper=async e=>{e=e.replace(".pdf","");const t=await fetchDom(e);e=t.getElementById("citeBibtexContent");if(e){var a=t.getElementById("title").innerText,e=bibtexToString(e.innerText);const l=bibtexToObject(e);var r=l.year,i=l.author.replace(/\s+/g," ").split(" and ").map(e=>e.split(",").map(e=>e.trim()).reverse().join(" ")).join(" and "),o=l.citationKey,n=findACLValue(t,"Venue"),s=findACLValue(t,"PDF");return{author:i,bibtex:e,id:`ACL-${n}-${r}_`+findACLValue(t,"Anthology ID"),key:o,note:`Accepted @ ${n} (${r})`,pdfLink:s,title:a,venue:n,year:r}}},makePNASPaper=async e=>{e=e.replace(".full.pdf","").replace(/\/doi\/e?pdf\//,"/doi/abs/");const t=await fetchDom(e);var a=t.getElementsByTagName("h1")[0].innerText,r=Array.from(t.querySelectorAll(".authors span[property='author'] a:not([property='email']):not(.orcid-id)")).filter(e=>!e.getAttribute("href").includes("mailto:")).map(e=>e.innerText).join(" and "),i=t.querySelector("span[property='datePublished']").innerText.match(/\d{4}/g)[0],o=`PNAS-${i}_`+(e.endsWith("/")?e.split("/").slice(-2):e.split("/").slice(-1))[0];const n=e.includes("/doi/pdf/")||e.includes("/doi/epdf/")?e.replace("/doi/epdf/","/doi/pdf/"):e.replace("/doi/abs/","/doi/pdf/").replace("/doi/full/","/doi/pdf/");var e=Array.from(t.querySelector(".self-citation").getElementsByTagName("a")).map(e=>e.getAttribute("href")).filter(e=>e.includes("https://doi.org"))[0].split("/").slice(-2).join("/"),s="doi:"+e;return{author:r,bibtex:bibtexToString(`@article{${s}, author={${r}}, title={${a}}, journal = {Proceedings of the National Academy of Sciences}, year={${i}}, doi={${e}}, eprint={${n}}, URL={${n.replace("/doi/pdf/","/doi/abs/")}} }`),id:o,key:s,note:`Published @ PNAS (${i})`,pdfLink:n,title:a,venue:"PNAS",year:i}},makeNaturePaper=async e=>{var t=(e=e.replace(".pdf","").split("#")[0])+".pdf",a=e.split("/").last();const r=await fetchDom(e);e=r.querySelector("h1.c-article-title").innerText;const i=Array.from(r.querySelectorAll("ul.c-article-author-list li")).map(e=>e.innerText.replace(/(\ ?,)|&|…|\d/g,"").split(/orcid/i)[0].trim()).filter(e=>0<e.length).join(" and ");var o=r.querySelector(".c-article-info-details").innerText.match(/\(\d{4}\)/)[0].replace(/\(|\)/g,""),n=r.querySelector(".c-article-info-details [data-test]").innerText,a=`Nature-${o}_`+a;let s;for(const d of[".c-bibliographic-information__citation",".c-bibliographic-information__value"])if(s=document.querySelector(d)?.innerText.split("https://doi.org/")[1])break;s=s||Array.from(r.getElementsByTagName("span")).map(e=>e.innerText).filter(e=>e.includes("https://doi.org"))[0];var l=""+i.split(" ")[1]+o+firstNonStopLowercase(e);let p={citationKey:l,entryType:"article",author:i,title:e,journal:n,year:o};s&&(p.doi=s,p.url="https://doi.org/"+s);var c=bibtexToString(p);return{author:i,bibtex:c,id:a,key:l,note:`Published @ ${n} (${o})`,pdfLink:t,title:e,venue:n,year:o}},makeACSPaper=async e=>{const t=(e=e.replace("pubs.acs.org/doi/pdf/","pubs.acs.org/doi/").split("?")[0]).replace("/abs/","/").split("/doi/")[1];console.log("doi: ",t);e=`https://pubs.acs.org/action/downloadCitation?doi=${t}&include=cit&format=bibtex&direct=true`,e=await fetchText(e);const a=bibtexToObject(e);var r=a.author.replaceAll("\n","").trim(),i=a.title.trim(),o=a.year.trim(),n=a.citationKey.trim(),s="https://pubs.acs.org/doi/pdf/"+t,l=`Published @ ${a.journal} (${a.year})`;return{author:r,bibtex:e,id:"ACS_"+t.replaceAll(".","").replaceAll("/",""),key:n,note:l,pdfLink:s,title:i,venue:a.journal,year:o}},makeIOPPaper=async e=>{(e=e.split("#")[0]).endsWith("/pdf")&&(e=e.slice(0,-4));const t=await fetchDom(e);var a=Array.from(t.querySelectorAll(".btn-multi-block a")).filter(e=>"BibTeX"===e.innerText).map(e=>e.getAttribute("href"))[0],a="https://"+parseUrl(e).host+a,a=await fetchText(a);const r=bibtexToObject(a);var i=r.author.replaceAll("\n","").trim(),o=r.title.trim(),n=r.year.trim(),s=r.citationKey.trim(),l=e+"/pdf",p=r.journal,c=`Published @ ${p} (${n})`;const d=e.split("/article/").last();return{author:i,bibtex:a,id:"IOPscience_"+d.replaceAll(".","").replaceAll("/",""),key:s,note:c,pdfLink:l,title:o,venue:p,year:n}},makeJMLRPaper=async e=>{var t=(e=(e=(e=e.includes("/papers/volume")?e.replace("/papers/volume","/papers/v"):e).endsWith(".pdf")?e.split("/").slice(0,-1).join("/"):e).replace(".html","")).split("/").last(),a=e+".bib",a=await fetchText(a);const{author:r,year:i,title:o,citationKey:n}=bibtexToObject(a);return{author:r,bibtex:a,id:`JMLR-${i}_`+t,key:n.trim(),note:`Published @ JMLR (${i})`,pdfLink:e.replace("/papers/v","/papers/volume")+`/${t}.pdf`,title:o,venue:"JMLR",year:i}},makePMCPaper=async e=>{var t=e.match(/PMC\d+/)[0].replace("PMC",""),a=e.split("PMC"+t)[0]+"PMC"+t;const r=await(await fetch(`https://api.ncbi.nlm.nih.gov/lit/ctxp/v1/pmc/?format=csl&id=${t}&download=true`)).json();var i=(r["epub-date"]||r.issued)["date-parts"][0][0],o=r.author.map(e=>e.given+" "+e.family).join(" and "),n=r["container-title"].split(" ").map(e=>e.capitalize()).join(" "),s=r.title,t=`PMC-${i}_`+t,l=""+r.author[0].family+i+firstNonStopLowercase(s),p=bibtexToString({entryType:"article",citationKey:l,journal:n,issn:r.ISSN,volume:r.volume,page:r.page,doi:r.DOI,PMID:r.PMID,PMCID:r.PMCID,publisher:r.publisher,author:o,title:s});let c;if(isPdfUrl(e))c=e;else{const h=r.DOI.split("/")[1].split("-");var e=h[0].match(/\d+/)[0],d=h[1].replace(h[1].match(/^0*/)[0],""),u=h[2].replace(h[2].match(/^0*/)[0],"");c=a+`/pdf/${e}_${d}_Article_${u}.pdf`}return{author:o,bibtex:p,id:t,key:l,note:`Published @ ${n} (${i})`,pdfLink:c,title:s,venue:n,year:i}},makePubMedPaper=async e=>{const t=await fetchDom(e.split("?")[0]),a=Array.from(t.getElementsByTagName("meta")).filter(e=>e.getAttribute("name")?.includes("citation_")),r=Object.fromEntries(a.map(e=>[e.getAttribute("name").replace("citation_",""),e.getAttribute("content")])),i=document.querySelector("div.authors-list").innerText.replace(/\d/gi,"").split(",").map(e=>e.trim()).join(" and ");var e=r.title,o=r.journal_title,n=r.date.split("/")[2],s=`PubMed-${n}_`+r.pmid,l=""+i.split(" and ")[0].split(" ").last()+n+firstNonStopLowercase(r.title),p={entryType:"article",citationKey:l,publisher:r.publisher,doi:r.doi,issn:r.issn,journal:o,year:n,author:i},p=bibtexToString(p),c=`Accepted @ ${journal} (${n})`;return{author:i,bibtex:p,id:s,key:l,note:c,pdfLink:"",title:e,venue:o,year:n}},makeIJCAIPaper=async e=>{const t=e.endsWith(".pdf")?e.replace(".pdf","").split("/").last().match(/[1-9]\d*/)[0]:e.split("/").last();e=e.match(/proceedings\/\d+/gi)[0].split("/")[1];const a=(await fetchText(`https://www.ijcai.org/proceedings/${e}/bibtex/`+t)).replace(/}\n/gi,"},\n");var r=bibtexToObject(a.split("\n").filter(e=>!/note\s+=/gi.test(e)).join("\n")),i=r.citationKey,o=r.title,r=flipAndAuthors(r.author),n=`IJCAI-${e}_`+t,s=`Accepted @ IJCAI (${e})`,l=t.padStart(4,0);return{author:r,bibtex:a,id:n,key:i,note:s,pdfLink:`https://www.ijcai.org/proceedings/${e}/${l}.pdf`,title:o,venue:"IJCAI",year:e}},makeACMPaper=async e=>{let t;t=isPdfUrl(e)?e:e.replace("/doi/","/doi/pdf/");var e=await fetchDom(e.replace("/doi/pdf/","/doi/")),{author:e,year:a,title:r,venue:i,key:o,doi:n,bibtex:s,note:l}=extractDataFromDCMetaTags(e);return{author:e,bibtex:s,id:`ACM-${a}_`+miniHash(n),key:o,note:l,pdfLink:t,title:r,venue:i,year:a}},makeIEEEPaper=async e=>{isPdfUrl(e)&&(e=`https://ieeexplore.ieee.org/document/${e.split("/stamp/stamp.jsp?tp=&arnumber=")[1].match(/\d+/)[0]}/`,console.log("url: ",e));const t=await fetchDom(e),a=JSON.parse(Array.from(t.getElementsByTagName("script")).filter(e=>e.innerHTML?.includes("metadata="))[0].innerHTML.split("metadata=")[1].trim().replace(/;$/,""));var r=a.title,i=a.authors.map(e=>e.name).join(" and "),o=a.publicationYear,e=""+parseUrl(e).origin+a.pdfUrl,n=a.publicationTitle,s=a.articleId;return{author:i,bibtex:bibtexToString({entryType:"article",citationKey:s,journal:n,volume:a.volume,pages:a.startPage+"-"+a.endPage,doi:a.doi,title:r,year:o,author:i}),id:`IEEE-${o}_`+s,key:s,note:`Accepted @ ${n} (${o})`,pdfLink:e,title:r,venue:n,year:o}},makeSpringerPaper=async t=>{const e=[...global.sourceExtras.springer.types,"content/pdf"];var a=e.find(e=>t.includes(`/${e}/`));if(!a)throw new Error(`Could not find Springer type for ${t} (known: ${e.join(", ")})`);var r=t.split(`/${a}/`)[1].split("?")[0].replace(".pdf",""),i=await fetchCrossRefDataForDoi(r);if(!i)throw new Error("Aborting Springer paper parsing, see error above");var{author:o,bibtex:n,citationKey:s,year:l,title:p,venue:c}=i;return{author:o,bibtex:n,id:`Springer-${l}_`+miniHash(r),key:s,note:`Published @ ${c} (${l})`,pdfLink:i.pdf??("content/pdf"===a?t:t.replace(`/${a}/`,"/content/pdf/")+".pdf"),title:p,venue:c,year:l,extra:{url:"https://doi.org/"+r}}},makeAPSPaper=async e=>{e=e.split("#")[0];var[t,a]=parseUrl(e).pathname.split("/").slice(1,3),r=e.split(`/${t}/${a}/`).last(),i=e.replace(`/${t}/${a}/`,`/${t}/export/`),i=await fetchText(i+"?type=bibtex&download=true"),o=bibtexToObject(i),e=e.replace(`/${t}/${a}/`,`/${t}/pdf/`),a=`APS-${o.year}_`+miniHash(r),t=o.journal??o.publisher,r=(await readJournalAbbreviations(),global.journalAbbreviations[miniHash(t)]??t),t=`Published @ ${r} (${o.year})`;return{author:flipAndAuthors(o.author),bibtex:i,id:a,key:o.citationKey,note:t,pdfLink:e,title:o.title,venue:r,year:o.year}},makeWileyPaper=async e=>{const t=e.replace(/\/doi\/(abs|epdf|full)\//g,"/doi/pdf/");e=t.replace("/doi/pdf/","/doi/abs/");const a=await fetchDom(e);var e=Array.from(a.querySelectorAll("meta[name=citation_author]")).map(e=>e.getAttribute("content")).join(" and "),r=a.querySelector("meta[name=citation_journal_title]").getAttribute("content"),i=a.querySelector("meta[name=citation_title]").getAttribute("content"),o=a.querySelector("meta[name=citation_publisher]").getAttribute("content"),n=a.querySelector("meta[name=citation_publication_date]")?.getAttribute("content")?.split("/")[0]??a.querySelector("meta[name=citation_online_date]")?.getAttribute("content")?.split("/")[0],s=a.querySelector("meta[name=citation_doi]").getAttribute("content"),l=`Published @ ${r} (${n})`,p=`Wiley-${n}_`+miniHash(s);return{author:e,bibtex:bibtexToString({citationKey:s,entryType:"article",title:i,author:e,year:n,doi:s,publisher:o,journal:r}),id:p,key:s,note:l,pdfLink:t,title:i,venue:r,year:n}},makeScienceDirectPaper=async e=>{var e=e.split("/pii/")[1].split("/")[0].split("#")[0].split("?")[0],t=await fetchText(`https://www.sciencedirect.com/sdfe/arp/cite?pii=${e}&format=text%2Fx-bibtex&withabstract=false`),{author:a,journal:r,year:i,title:o,citationKey:n}=bibtexToObject(t),s=`Published @ ${r} (${i})`;return{author:a,bibtex:t,id:`ScienceDirect-${i}_`+miniHash(e),key:n,note:s,pdfLink:"https://reader.elsevier.com/reader/sd/pii/"+e,title:o,venue:r??"Science Direct",year:i}},makeSciencePaper=async e=>{let t,a,r,i,o,n,s,l,p,c,d;n="https://science.org/doi/pdf/"+(c=(c=noParamUrl(e).split("/doi/")[1]).startsWith("10.")?c:c.split("/").slice(1).join("/")),d="https://science.org/doi/full/"+c;var e=await fetchCrossRefDataForDoi(c);return e?({author:t,bibtex:a,title:s,venue:l,year:p}=e,i=e.citationKey,o=`Published @ ${l} (${p})`):(e=await fetchDom(d),{author:t,year:p,publisher,title:s,venue:l,key:i,bibtex:a,note:o}=extractDataFromDCMetaTags(e)),r=`Science-${p}_`+miniHash(c),{author:t,bibtex:a,id:r,key:i,note:o,pdfLink:n,title:s,venue:l,year:p}},tryPWCMatch=async e=>{e={type:"papersWithCode",pwcPrefs:await getStorage("pwcPrefs")??{},paper:e};return sendMessageToBackground(e)},tryCrossRef=async e=>{try{var t="https://api.crossref.org/works?rows=1&mailto=schmidtv%40mila.quebec&select=event%2Ctitle&query.title="+encodeURI(e.title);const r=await fetch(t).then(e=>e.json());if("ok"!==r.status)return log(`[Crossref] ${t} returned `+r.message.status),{note:null};if(0===r.message.items.length)return{note:null};if(r.message.items[0].title[0].toLowerCase().replaceAll("\n"," ").replaceAll(/\s\s+/g," ")!==e.title.toLowerCase().replaceAll("\n"," ").replaceAll(/\s\s+/g," "))return{note:null};if(!r.message.items[0].event||!r.message.items[0].event.name)return{note:null};info("Found a CrossRef match");var a=r.message.items[0].event.name.trim();return{venue:a,note:`Accepted @ ${a} -- [crossref.org]`}}catch(e){return logError("[Crossref]",e),{note:null}}},tryDBLP=async e=>{try{var t,a,r,i,o=encodeURI(e.title);const n=await fetch(`https://dblp.org/search/publ/api?q=${o}&format=json`).then(e=>e.json());if(!(n.result&&n.result.hits&&n.result.hits.hit&&n.result.hits.hit.length))return{note:null};for(const s of n.result.hits.hit.sort((e,t)=>parseInt(e.info.year,10)-parseInt(t.info.year,10)))if(decodeHtml(s.info.title.toLowerCase().replaceAll("\n"," ").replaceAll(".","").replaceAll(/\s\s+/g," "))===e.title.toLowerCase().replaceAll("\n"," ").replaceAll(".","").replaceAll(/\s\s+/g," ")&&"CoRR"!==s.info.venue)return info("Found a DBLP match"),t=await fetchText(s.info.url+".bib"),a=miniHash(s.info.venue),await readJournalAbbreviations(),r=(global.journalAbbreviations[a]??s.info.venue).trim(),i=s.info.year,s.info.url,{venue:r,note:`Accepted @ ${r} ${i} -- [dblp.org]`,bibtex:t};return{note:null}}catch(e){return logError("[DBLP]",e),{note:null}}},trySemanticScholar=async e=>{try{var t=await fetchJSON(`https://api.semanticscholar.org/graph/v1/paper/search?query=${encodeURI(e.title)}&fields=title,venue,year&limit=50`);if(t&&t.data&&0<t.data.length)for(const i of t.data)if(miniHash(i.title)===miniHash(e.title)&&i.venue&&"arxiv"!==i.venue.toLowerCase()){info("Found a Semantic Scholar match");let e=i.venue.trim().replace(/^\d{4}/,"").trim().capitalize(!0);var a=i.year,r=`Accepted @ ${e} (${a}) -- [semanticscholar.org]`;return{venue:e=e.toLowerCase(),note:r}}}catch(e){logError("[SemanticScholar]",e)}},tryPreprintMatch=async(e,t=!1)=>{let a,r,i,o;if(t&&(t={type:"papersWithCode",pwcPrefs:await getStorage("pwcPrefs")??{},paper:e},t=await sendMessageToBackground(t),pwcUrl=!e.codeLink&&t?.url,pwcNote=!e.note&&t?.note,pwcVenue=!e.venue&&t?.venue,pwcUrl&&(o=t),pwcNote&&(a=pwcNote),pwcVenue&&(r=pwcVenue)),r)return{note:a,venue:r,bibtex:i};var t=await tryDBLP(e);return a=t.note,r=t.venue,i=t.bibtex,a||(log("[DBLP] No publication found"),t=await trySemanticScholar(e),a=t?.note,r=t?.venue),a||(log("[SemanticScholar] No publication found"),t=await tryCrossRef(e),a=t.note,r=t.venue),a||log("[CrossRef] No publication found"),{note:a,venue:r,bibtex:i,code:o}},initPaper=async e=>{e.note||(e.note=""),e.md=`[${e.title}](${e.pdfLink})`,e.tags=[],e.codeLink="",e.favorite=!1,e.favoriteDate="",e.addDate=(new Date).toJSON(),e.lastOpenDate=e.addDate,e.count=1,e.code={};for(const t in e)e.hasOwnProperty(t)&&"string"==typeof e[t]&&(e[t]=e[t].trim());return e=await autoTagPaper(e),validatePaper(e),e},autoTagPaper=async a=>{try{var e,r,i=await getStorage("autoTags");if(!i||!i.length)return a;let t=new Set;for(const o of i)o.tags?.length&&(o.title||o.author)&&(e=!o.title||new RegExp(o.title,"i").test(a.title),r=!o.author||new RegExp(o.author,"i").test(a.author),e&&r&&o.tags.forEach(e=>t.add(e)));return a.tags=Array.from(t).sort(),a.tags.length&&log("Automatically adding tags:",a.tags),a}catch(e){return log("Error auto-tagging:",e),log("Paper:",a),a}},makePaper=async(e,t)=>{let a;if(e.arxiv)(a=await makeArxivPaper(t)).source="arxiv";else if(e.neurips)(a=await makeNeuripsPaper(t)).source="neurips";else if(e.cvf)(a=await makeCVFPaper(t)).source="cvf";else if(e.openreview)(a=await makeOpenReviewPaper(t)).source="openreview";else if(e.biorxiv)(a=await makeBioRxivPaper(t)).source="biorxiv";else if(e.pmlr)(a=await makePMLRPaper(t)).source="pmlr";else if(e.acl)(a=await makeACLPaper(t))&&(a.source="acl");else if(e.pnas)(a=await makePNASPaper(t))&&(a.source="pnas");else if(e.nature)(a=await makeNaturePaper(t))&&(a.source="nature");else if(e.acs)(a=await makeACSPaper(t))&&(a.source="acs");else if(e.iop)(a=await makeIOPPaper(t))&&(a.source="iop");else if(e.jmlr)(a=await makeJMLRPaper(t))&&(a.source="jmlr");else if(e.pmc)(a=await makePMCPaper(t))&&(a.source="pmc");else if(e.ijcai)(a=await makeIJCAIPaper(t))&&(a.source="ijcai");else if(e.acm)(a=await makeACMPaper(t))&&(a.source="acm");else if(e.ieee)(a=await makeIEEEPaper(t))&&(a.source="ieee");else if(e.springer)(a=await makeSpringerPaper(t))&&(a.source="springer");else if(e.aps)(a=await makeAPSPaper(t))&&(a.source="aps");else if(e.wiley)(a=await makeWileyPaper(t))&&(a.source="wiley");else if(e.sciencedirect)(a=await makeScienceDirectPaper(t))&&(a.source="sciencedirect");else{if(!e.science)throw new Error("Unknown paper source: "+JSON.stringify({is:e,url:t}));(a=await makeSciencePaper(t))&&(a.source="science")}if(void 0!==a)return initPaper(a)},findFuzzyPaperMatch=(e,t)=>{t=miniHash(t.title);if(e.hasOwnProperty(t)){const a=e[t];e=a.find(t=>!global.preprintSources.some(e=>t.toLowerCase().startsWith(e)));return e?e:a[0]}return null};"undefined"!=typeof module&&null!=module.exports&&(dummyModule=module,dummyModule.exports={decodeHtml:decodeHtml,flipAuthor:flipAuthor,flipAndAuthors:flipAndAuthors,fetchArxivXML:fetchArxivXML,fetchCvfHTML:fetchCvfHTML,fetchOpenReviewNoteJSON:fetchOpenReviewNoteJSON,fetchOpenReviewForumJSON:fetchOpenReviewForumJSON,fetchDom:fetchDom,fetchText:fetchText,makeArxivPaper:makeArxivPaper,makeNeuripsPaper:makeNeuripsPaper,makeCVFPaper:makeCVFPaper,makeOpenReviewBibTex:makeOpenReviewBibTex,makeOpenReviewPaper:makeOpenReviewPaper,makeBioRxivPaper:makeBioRxivPaper,makePMLRPaper:makePMLRPaper,findACLValue:findACLValue,makeACLPaper:makeACLPaper,makePNASPaper:makePNASPaper,makeNaturePaper:makeNaturePaper,makeACSPaper:makeACSPaper,makeIOPPaper:makeIOPPaper,makeJMLRPaper:makeJMLRPaper,makePMCPaper:makePMCPaper,makePubMedPaper:makePubMedPaper,makeIJCAIPaper:makeIJCAIPaper,makeACMPaper:makeACMPaper,makeIEEEPaper:makeIEEEPaper,makeSpringerPaper:makeSpringerPaper,makeAPSPaper:makeAPSPaper,makeWileyPaper:makeWileyPaper,tryCrossRef:tryCrossRef,tryDBLP:tryDBLP,tryPreprintMatch:tryPreprintMatch,tryPWCMatch:tryPWCMatch,initPaper:initPaper,autoTagPaper:autoTagPaper,makePaper:makePaper,findFuzzyPaperMatch:findFuzzyPaperMatch,fetchCrossRefDataForDoi:fetchCrossRefDataForDoi,fetchSemanticsScholarDataForDoi:fetchSemanticsScholarDataForDoi});