...
`+`url={${e}},
`)+`year={${s}}
`+"}",a=bibtexToString(a));r=e.replace("/hash/","/file/").replace("-Abstract.html","-Paper.pdf"),i=`NeurIPS-${s}_`+t.slice(0,8),e="NeurIPS",t=`Accepted @ ${e} (${s})`;return{author:n,bibtex:a,id:i,key:l,note:t,pdfLink:r,title:o,venue:e,year:s}},makeCVFPaper=async e=>{var t=await fetchCvfHTML(e),t=(new DOMParser).parseFromString(t.replaceAll("\n",""),"text/html"),r=t.getElementById("papertitle").innerText.trim(),i=t.querySelector("#authors i").innerText.split(",").map(e=>e.trim()).join(" and "),{year:a,id:n,conf:o}=parseCVFUrl(e);let s="";if(e.endsWith(".pdf"))s=e;else{let e=[...t.getElementsByTagName("a")].filter(e=>"pdf"===e.innerText)[0].getAttribute("href");(e=e.startsWith("../")?e.replaceAll("../",""):e).startsWith("/")||(e="/"+e),s="http://openaccess.thecvf.com"+e}var e=o,o=`Accepted @ ${e} (${a})`,t=bibtexToString(t.querySelector(".bibref").innerText),l=t.split("{")[1].split(",")[0];return{author:i,bibtex:t,id:n,key:l,note:o,pdfLink:s,title:r,venue:e,year:a}},makeOpenReviewBibTex=(e,t)=>{var r=e.content.title,i=e.content.authors.join(" and "),a=e.cdate?new Date(e.cdate).getFullYear():"0000",e=(e.cdate||log("makeOpenReviewBibTex: no cdate found in",e),e.content.authors[0].split(" ").last()),n="";return n+`@inproceedings{${e=(e+=a)+firstNonStopLowercase(r)},
`+`title={${r}},
`+`author={${i}},
`+`year={${a}},
`+`url={${t}},
`+"}"},makeOpenReviewPaper=async e=>{var t=await getOpenReviewNoteJSON(e),r=await getOpenReviewForumJSON(e),t=t.notes[0],r=r.notes,i=t.content.title,a=t.content.authors.join(" and "),n=bibtexToString(t.content._bibtex||makeOpenReviewBibTex(t,e)),o=bibtexToObject(n),s=o.citationKey,o=o.year;let l;l=t.pdf?"https://openreview.net/pdf?id="+t.id:(t.html||e).replace("/forum?id=","/pdf?id=");e=t.invitation.split("/");let c=e[0].split(".")[0],p=e.slice(1).join("/").split("-")[0].replaceAll("/"," ").replace(" Conference",""),u=c,d=!1;global.overrideORConfs.hasOwnProperty(c)&&(u=global.overrideORConfs[c],d=!0),d&&(p=p.replace(u,""),c=u);e=(c+" "+p).replace(/ \d\d\d\d/g,"").replace(/\s\s+/g," "),t=`OR-${c}-${o}_`+t.id;let h,f,m,g="";return(h=r.filter(e=>e&&e.content&&-1<["Final Decision","Paper Decision","Acceptance Decision"].indexOf(e.content.title)))&&0<h.length&&(f=h[0].content.decision.split(" ").map((e,t)=>0===t?e+"ed":e).join(" "),m=f+` @ ${e} (${o})`,f.toLowerCase().indexOf("rejected")<0)&&(g=e),{author:a,bibtex:n,id:t,key:s,note:m="Anonymous"===a&&"Rejected"!=f?`Under review @ ${e} (${o}) (${(new Date).toLocaleDateString()})`:m,pdfLink:l,title:i,venue:g,year:o}},makeBioRxivPaper=async e=>{var t=e.replace(".full.pdf",""),r="https://api.biorxiv.org//details/biorxiv/"+e.split("/").slice(-2).join("/").replace(".full.pdf","").split("v")[0],i=await fetch(r).then(e=>e.json());if("ok"!==i.messages[0].status)throw new Error(r+" returned "+i.messages[0].status);var r=i.collection.last(),i=await fetchText(t),t=(new DOMParser).parseFromString(i.replaceAll("\n",""),"text/html").querySelector(".bibtext a").getAttribute("href"),i=bibtexToString(await(await fetch(t)).text()),t=extractAuthor(i),a=await parseIdFromUrl(e),n=i.split("\n")[0].split("{")[1].replace(",","").trim();return{author:t,bibtex:i,id:a,key:n,note:"",pdfLink:cleanBiorxivURL(e)+".full.pdf",title:r.title,venue:"",year:r.date.split("-")[0]}},makePMLRPaper=async e=>{var t=e.split("/").last().split(".")[0],r=await parseIdFromUrl(e),e=e.includes(".html")?e:e.split("/").slice(0,-2).join("/")+`/${t}.html`,i=e.replace(".html","")+`/${t}.pdf`,e=await fetchDom(e),a=(e.getElementById("button-bibtex1").getAttribute("onclick").match(/https.+\.bib/)[0],e.getElementById("bibtex").innerText.replaceAll("\t"," ").replaceAll(/\s\s+/g," "));let n=a;for(const c of a.match(/,\ ?\w+ ?= ?{/g))n=n.replace(c,c.replace(", ",",\n    ").replace(" = ","="));n.endsWith("}}")&&(n=n.slice(0,-2)+"}\n}"),n=bibtexToString(n);var a=extractAuthor(n),e=e.getElementsByTagName("h1")[0].innerText,o=extractBibtexValue(n,"year");let s=extractBibtexValue(n,"booktitle").replaceAll("Proceedings of the",""),l=s;note=`Accepted @ ${l} (${o})`;for(const p in global.overridePMLRConfs)if(s.includes(p)){l=global.overridePMLRConfs[p],s=l+" "+o,note="Accepted @ "+s;break}return{author:a,bibtex:n,id:r,key:t,note:note,pdfLink:i,title:e,venue:l,year:o}},findACLValue=(e,t)=>{return queryAll(e,"dt").filter(e=>e.innerText.includes(t))[0].nextElementSibling.innerText},makeACLPaper=async e=>{e=e.replace(".pdf","");var t,r,i,a,n,o,e=await fetchDom(e),s=e.getElementById("citeBibtexContent");if(s)return t=e.getElementById("title").innerText,s=bibtexToString(s.innerText),r=(a=bibtexToObject(s)).year,i=a.author.replace(/\s+/g," ").split(" and ").map(e=>e.split(",").map(e=>e.trim()).reverse().join(" ")).join(" and "),a=a.citationKey,n=findACLValue(e,"Venue"),o=findACLValue(e,"PDF"),{author:i,bibtex:s,id:`ACL-${n}-${r}_`+findACLValue(e,"Anthology ID"),key:a,note:`Accepted @ ${n} (${r})`,pdfLink:o,title:t,venue:n,year:r}},makePNASPaper=async e=>{e=e.replace(".full.pdf","").replace(/\/doi\/e?pdf\//,"/doi/abs/");var t=await fetchDom(e),r=t.getElementsByTagName("h1")[0].innerText,i=queryAll(t,".authors span[property='author'] a:not([property='email']):not(.orcid-id)").filter(e=>!e.getAttribute("href").includes("mailto:")).map(e=>e.innerText).join(" and "),a=t.querySelector("span[property='datePublished']").innerText.match(/\d{4}/g)[0],n=`PNAS-${a}_`+(e.endsWith("/")?e.split("/").slice(-2):e.split("/").slice(-1))[0],e=e.includes("/doi/pdf/")||e.includes("/doi/epdf/")?e.replace("/doi/epdf/","/doi/pdf/"):e.replace("/doi/abs/","/doi/pdf/").replace("/doi/full/","/doi/pdf/"),t=[...t.querySelector(".core-container").getElementsByTagName("a")].map(e=>e.getAttribute("href")).filter(e=>e?.includes("https://doi.org"))[0].split("/").slice(-2).join("/"),o="doi:"+t;return{author:i,bibtex:bibtexToString(`@article{${o}, author={${i}}, title={${r}}, journal = {Proceedings of the National Academy of Sciences}, year={${a}}, doi={${t}}, eprint={${e}}, URL={${e.replace("/doi/pdf/","/doi/abs/")}} }`),id:n,key:o,note:`Published @ PNAS (${a})`,pdfLink:e,title:r,venue:"PNAS",year:a}},makeNaturePaper=async e=>{var t=(e=e.replace(".pdf","").split("#")[0])+".pdf",r=e.split("/").last(),e=await fetchDom(e),i=e.querySelector("h1.c-article-title").innerText,a=queryAll(e,"ul.c-article-author-list li").map(e=>e.innerText.replace(/(\ ?,)|&|…|\d/g,"").split(/orcid/i)[0].trim()).filter(e=>0<e.length).join(" and "),n=e.querySelector(".c-article-info-details").innerText.match(/\(\d{4}\)/)[0].replace(/\(|\)/g,""),o=e.querySelector(".c-article-info-details [data-test]").innerText,r=`Nature-${n}_`+r;let s;for(const c of[".c-bibliographic-information__citation",".c-bibliographic-information__value"])if(s=document.querySelector(c)?.innerText.split("https://doi.org/")[1])break;s=s||[...e.getElementsByTagName("span")].map(e=>e.innerText).filter(e=>e.includes("https://doi.org"))[0];var e=""+a.split(" ")[1]+n+firstNonStopLowercase(i),l={citationKey:e,entryType:"article",author:a,title:i,journal:o,year:n},l=(s&&(l.doi=s,l.url="https://doi.org/"+s),bibtexToString(l));return{author:a,bibtex:l,id:r,key:e,note:`Published @ ${o} (${n})`,pdfLink:t,title:i,venue:o,year:n}},makeACSPaper=async e=>{var e=(e=e.replace("pubs.acs.org/doi/pdf/","pubs.acs.org/doi/").split("?")[0]).replace("/abs/","/").split("/doi/")[1],t=(console.log("doi: ",e),`https://pubs.acs.org/action/downloadCitation?doi=${e}&include=cit&format=bibtex&direct=true`),t=await fetchText(t),r=bibtexToObject(t),i=r.author.replaceAll("\n","").trim(),a=r.title.trim(),n=r.year.trim(),o=r.citationKey.trim(),s="https://pubs.acs.org/doi/pdf/"+e,l=`Published @ ${r.journal} (${r.year})`;return{author:i,bibtex:t,id:"ACS_"+e.replaceAll(".","").replaceAll("/",""),key:o,note:l,pdfLink:s,title:a,venue:r.journal,year:n}},makeIOPPaper=async e=>{(e=e.split("#")[0]).endsWith("/pdf")&&(e=e.slice(0,-4));var t=await fetchDom(e),t=queryAll(t,".btn-multi-block a").filter(e=>"BibTeX"===e.innerText).map(e=>e.getAttribute("href"))[0],t="https://"+parseUrl(e).host+t,t=await fetchText(t),r=bibtexToObject(t),i=r.author.replaceAll("\n","").trim(),a=r.title.trim(),n=r.year.trim(),o=r.citationKey.trim(),s=e+"/pdf",r=r.journal,l=`Published @ ${r} (${n})`;return{author:i,bibtex:t,id:"IOPscience_"+e.split("/article/").last().replaceAll(".","").replaceAll("/",""),key:o,note:l,pdfLink:s,title:a,venue:r,year:n}},makeJMLRPaper=async e=>{var t=(e=(e=(e=e.includes("/papers/volume")?e.replace("/papers/volume","/papers/v"):e).endsWith(".pdf")?e.split("/").slice(0,-1).join("/"):e).replace(".html","")).split("/").last(),r=e+".bib",r=await fetchText(r),{author:i,year:a,title:n,citationKey:o}=bibtexToObject(r);return{author:i,bibtex:r,id:`JMLR-${a}_`+t,key:o.trim(),note:`Published @ JMLR (${a})`,pdfLink:e.replace("/papers/v","/papers/volume")+`/${t}.pdf`,title:n,venue:"JMLR",year:a}},makePMCPaper=async e=>{var t=e.match(/PMC\d+/)[0].replace("PMC",""),r=e.split("PMC"+t)[0]+"PMC"+t,i=await(await fetch(`https://api.ncbi.nlm.nih.gov/lit/ctxp/v1/pmc/?format=csl&id=${t}&download=true`)).json(),a=(i["epub-date"]||i.issued)["date-parts"][0][0],n=i.author.map(e=>e.given+" "+e.family).join(" and "),o=i["container-title"].split(" ").map(e=>e.capitalize()).join(" "),s=i.title,t=`PMC-${a}_`+t,l=""+i.author[0].family+a+firstNonStopLowercase(s);let c;return{author:n,bibtex:bibtexToString({entryType:"article",citationKey:l,journal:o,issn:i.ISSN,volume:i.volume,page:i.page,doi:i.DOI,PMID:i.PMID,PMCID:i.PMCID,publisher:i.publisher,author:n,title:s}),id:t,key:l,note:`Published @ ${o} (${a})`,pdfLink:c=isPdfUrl(e)?e:r+`/pdf/${(n=i.DOI.split("/")[1].split("-"))[0].match(/\d+/)[0]}_${n[1].replace(n[1].match(/^0*/)[0],"")}_Article_${n[2].replace(n[2].match(/^0*/)[0],"")}.pdf`,title:s,venue:o,year:a}},makePubMedPaper=async e=>{var e=[...(await fetchDom(e.split("?")[0])).getElementsByTagName("meta")].filter(e=>e.getAttribute("name")?.includes("citation_")),e=Object.fromEntries(e.map(e=>[e.getAttribute("name").replace("citation_",""),e.getAttribute("content")])),t=document.querySelector("div.authors-list").innerText.replace(/\d/gi,"").split(",").map(e=>e.trim()).join(" and "),r=e.title,i=e.journal_title,a=e.date.split("/")[2],n=`PubMed-${a}_`+e.pmid,o=""+t.split(" and ")[0].split(" ").last()+a+firstNonStopLowercase(e.title),e={entryType:"article",citationKey:o,publisher:e.publisher,doi:e.doi,issn:e.issn,journal:i,year:a,author:t};return{author:t,bibtex:bibtexToString(e),id:n,key:o,note:`Accepted @ ${journal} (${a})`,pdfLink:"",title:r,venue:i,year:a}},makeIJCAIPaper=async e=>{var t=e.endsWith(".pdf")?e.replace(".pdf","").split("/").last().match(/[1-9]\d*/)[0]:e.split("/").last(),e=e.match(/proceedings\/\d+/gi)[0].split("/")[1],r=(await fetchText(`https://www.ijcai.org/proceedings/${e}/bibtex/`+t)).replace(/}\n/gi,"},\n"),i=bibtexToObject(r.split("\n").filter(e=>!/note\s+=/gi.test(e)).join("\n")),a=i.citationKey,n=i.title;return{author:flipAndAuthors(i.author),bibtex:r,id:`IJCAI-${e}_`+t,key:a,note:`Accepted @ IJCAI (${e})`,pdfLink:`https://www.ijcai.org/proceedings/${e}/${t.padStart(4,0)}.pdf`,title:n,venue:"IJCAI",year:e}},makeACMPaper=async e=>{let t;t=isPdfUrl(e)?e:e.replace(/\/doi\/?(abs|full)?\//,"/doi/pdf/");var r=await fetchDom(e.replace("/doi/pdf/","/doi/"));let i,a,n,o,s,l,c,p;var u=extractDataFromDCMetaTags(r),r=(u?{author:i,year:a,title:n,venue:o,key:s,doi:l,bibtex:c,note:p}=u:(n=r.querySelector(".citation__title").innerText,i=queryAll(r,"ul[ariaa-label='authors'] li.loa__item .loa__author-name").map(e=>e.innerText.replace(",","").trim()).join(" and "),u=r.querySelector(".issue-item__detail a").innerText,o=u.split("'")[0].trim(),a="20"+u.split("'")[1].split(":")[0].trim(),l=t.split("/doi/pdf/")[1],p=`Accepted @ ${o} (${a})`,s=l,c=bibtexToString({entryType:"article",citationKey:l,journal:o,author:i,title:n,year:a,publisher:"Association for Computing Machinery",address:"New York, NY, USA",url:e.replace("/doi/pdf/","/doi/")})),`ACM-${a}_`+miniHash(l));return{author:i,bibtex:c,id:r,key:s,note:p,pdfLink:t,title:n,venue:o,year:a}},makeIEEEPaper=async e=>{isPdfUrl(e)&&(e=`https://ieeexplore.ieee.org/document/${e.split("/stamp/stamp.jsp?tp=&arnumber=")[1].match(/\d+/)[0]}/`,console.log("url: ",e));var t=await fetchDom(e),t=JSON.parse([...t.getElementsByTagName("script")].filter(e=>e.innerHTML?.includes("metadata="))[0].innerHTML.split("metadata=")[1].split(/};\s*/)[0]+"}"),r=t.title,i=t.authors.map(e=>e.name).join(" and "),a=t.publicationYear,e=""+parseUrl(e).origin+t.pdfUrl,n=t.publicationTitle,o=t.articleId;return{author:i,bibtex:bibtexToString({entryType:"article",citationKey:o,journal:n,volume:t.volume,pages:t.startPage+"-"+t.endPage,doi:t.doi,title:r,year:a,author:i}),id:`IEEE-${a}_`+o,key:o,note:`Accepted @ ${n} (${a})`,pdfLink:e,title:r,venue:n,year:a}},makeSpringerPaper=async t=>{var e=[...global.sourceExtras.springer.types,"content/pdf"],r=e.find(e=>t.includes(`/${e}/`));if(!r)throw new Error(`Could not find Springer type for ${t} (known: ${e.join(", ")})`);var i,a,n,o,s,l,e=t.split(`/${r}/`)[1].split("?")[0].replace(".pdf",""),c=await fetchCrossRefDataForDoi(e);if(c)return{author:i,bibtex:a,citationKey:n,year:o,title:s,venue:l}=c,{author:i,bibtex:a,id:`Springer-${o}_`+miniHash(e),key:n,note:`Published @ ${l} (${o})`,pdfLink:c.pdf??("content/pdf"===r?t:t.replace(`/${r}/`,"/content/pdf/")+".pdf"),title:s,venue:l,year:o,extra:{url:"https://doi.org/"+e}};throw new Error("Aborting Springer paper parsing, see error above")},makeAPSPaper=async e=>{e=e.split("#")[0];var[t,r]=parseUrl(e).pathname.split("/").slice(1,3),i=e.split(`/${t}/${r}/`).last(),a=e.replace(`/${t}/${r}/`,`/${t}/export/`),a=await fetchText(a+"?type=bibtex&download=true"),n=bibtexToObject(a),e=e.replace(`/${t}/${r}/`,`/${t}/pdf/`),r=`APS-${n.year}_`+miniHash(i),t=n.journal??n.publisher,i=(await readJournalAbbreviations(),global.journalAbbreviations[miniHash(t)]??t),t=`Published @ ${i} (${n.year})`;return{author:flipAndAuthors(n.author),bibtex:a,id:r,key:n.citationKey,note:t,pdfLink:e,title:n.title,venue:i,year:n.year}},makeWileyPaper=async e=>{var e=e.replace(/\/doi\/(abs|epdf|full)\//g,"/doi/pdf/"),t=e.replace("/doi/pdf/","/doi/abs/"),t=await fetchDom(t),r=queryAll("meta[name=citation_author]").map(e=>e.getAttribute("content")).join(" and "),i=t.querySelector("meta[name=citation_journal_title]").getAttribute("content"),a=t.querySelector("meta[name=citation_title]").getAttribute("content"),n=t.querySelector("meta[name=citation_publisher]").getAttribute("content"),o=t.querySelector("meta[name=citation_publication_date]")?.getAttribute("content")?.split("/")[0]??t.querySelector("meta[name=citation_online_date]")?.getAttribute("content")?.split("/")[0],t=t.querySelector("meta[name=citation_doi]").getAttribute("content"),s=`Published @ ${i} (${o})`,l=`Wiley-${o}_`+miniHash(t);return{author:r,bibtex:bibtexToString({citationKey:t,entryType:"article",title:a,author:r,year:o,doi:t,publisher:n,journal:i}),id:l,key:t,note:s,pdfLink:e,title:a,venue:i,year:o}},makeScienceDirectPaper=async e=>{var e=e.split("/pii/")[1].split("/")[0].split("#")[0].split("?")[0],t=await fetchText(`https://www.sciencedirect.com/sdfe/arp/cite?pii=${e}&format=text%2Fx-bibtex&withabstract=false`),{author:r,journal:i,year:a,title:n,citationKey:o}=bibtexToObject(t),s=`Published @ ${i} (${a})`;return{author:r,bibtex:t,id:`ScienceDirect-${a}_`+miniHash(e),key:o,note:s,pdfLink:"https://reader.elsevier.com/reader/sd/pii/"+e,title:n,venue:i??"Science Direct",year:a}},makeSciencePaper=async e=>{let t,r,i,a,n,o,s,l,c,p,u;o="https://science.org/doi/pdf/"+(p=(p=noParamUrl(e).split("/doi/")[1]).startsWith("10.")?p:p.split("/").slice(1).join("/")),u="https://science.org/doi/full/"+p;var e=await fetchCrossRefDataForDoi(p);return e?({author:t,bibtex:r,title:s,venue:l,year:c}=e,a=e.citationKey,n=`Published @ ${l} (${c})`):(e=await fetchDom(u),{author:t,year:c,publisher,title:s,venue:l,key:a,bibtex:r,note:n}=extractDataFromDCMetaTags(e)),i=`Science-${c}_`+miniHash(p),{author:t,bibtex:r,id:i,key:a,note:n,pdfLink:o,title:s,venue:l,year:c}},makeFrontiersPaper=async e=>{e=e.replace(/\/pdf$/,"/full");var t=noParamUrl(e).split("/articles/")[1].split("/full")[0],r=await fetchText(`https://www.frontiersin.org/articles/${t}/bibTex`),r=Object.fromEntries(Object.entries(bibtexToObject(r)).map(([e,t])=>["citationKey"===e||"entryType"===e?e:e.toLowerCase(),t])),{author:i,journal:a,year:n,title:o,citationKey:s}=(r.author=flipAndAuthors(r.author),delete r.abstract,r),l=`Published @ ${a} (${n})`;return{author:i,bibtex:bibtexToString(r),id:`Frontiers-${n}_`+miniHash(t),key:s,note:l,pdfLink:e.replace(/\/full$/,"/pdf"),title:o,venue:a,year:n}},makeIHEPPaper=async t=>{let r,i;if(t.includes("/files/")){const d=t.split("/files/")[1].split("/")[0];var a="https://inspirehep.net/api/literature?q=documents.key:"+d,a=await fetchJSON(a);if(!(r=a.hits.hits.find(e=>!!e.metadata.documents.find(e=>e.key===d))))return void warn("Could not find an Inspire HEP record for the url",t);i=r.metadata.control_number}else i=t.match(/\/literature\/(\d+)/)[1];if(i){var a=await fetchText(`https://inspirehep.net/api/literature/${i}?format=bibtex`),n=(r=r||await fetchJSON(`https://inspirehep.net/api/literature/${i}?format=json`),bibtexToObject(a));let e=n.title??r.metadata.titles[0].title;e.startsWith("{")&&e.endsWith("}")&&(e=e.slice(1,-1));var o=r.metadata.documents?.[0]?.url??t,s=flipAndAuthors(n.author),l=n.year??r.created.split("-")[0],c="IHEP-"+i,p=n.journal??"Inspire HEP",u=n.citationKey,n=n.doi??"";return{author:s,bibtex:a,id:c,key:u,note:`Published @ ${p} (${l})`,pdfLink:o,title:e,venue:p,year:l,doi:n}}warn("Could not find an Inspire HEP id for the url",t)},makePLOSPaper=async e=>{var t=e.split("?id=").last().split("&")[0],{bibtex:r,key:i,author:a,venue:n,title:o,note:s,year:l}=await fetchBibtex(e.split("/article")[0]+"/article/citation/bibtex?id="+t),c=e.split("/article")[0]+`/article/file?id=${t}&type=printable`,e=e.split("journals.plos.org/")[1].split("/")[0];return{author:flipAndAuthors(a),bibtex:r,id:`PLOS-${e}_`+miniHash(t),key:i,note:s,pdfLink:c,title:o,venue:n,year:l,doi:t}},makeRSCPaper=async e=>{var t=noParamUrl(e).split("/").last(),r=e.split("/").find(e=>"articlehtml"===e||"articlepdf"===e||"articlelanding"===e).replace("article",""),e="articlepdf"===r?e:e.replace(`/article${r}/`,"/articlepdf/"),{bibtex:r,key:i,author:a,venue:n,title:o,note:s,year:l,doi:c}=await fetchBibtex(`https://pubs.rsc.org/en/content/formatedresult?markedids=${t}&downloadtype=article&managertype=bibtex`);return{author:flipAndAuthors(a),bibtex:r,id:`RSC-${n.replaceAll(" ","")}_`+miniHash(t),key:i,note:s,pdfLink:e,title:o,venue:n,year:l,doi:c}},tryPWCMatch=async e=>{let t;var r={type:"papersWithCode",pwcPrefs:await getStorage("pwcPrefs")??{},paper:e},{url:r,note:i,venue:a,pubYear:n}=await sendMessageToBackground(r)??{};return r&&!e.codeLink?log("[PapersWithCode] Discovered a code repository:",r):log("[PapersWithCode] No code repository found"),a&&!e.venue?(log("[PapersWithCode] Found a publication venue:",a),e=bibtexToObject(e.bibtex),t=bibtexToString({...e,year:n,journal:a})):log("[PapersWithCode] No publication found"),{codeLink:r,note:i,venue:a,bibtex:t}},tryCrossRef=async e=>{try{var t="https://api.crossref.org/works?rows=1&mailto=schmidtv%40mila.quebec&select=event%2Ctitle&query.title="+encodeURI(e.title),r=await fetch(t).then(e=>e.json());if("ok"!==r.status)return log(`[Crossref] ${t} returned `+r.message.status),{note:null};if(0===r.message.items.length)return{note:null};if(r.message.items[0].title[0].toLowerCase().replaceAll("\n"," ").replaceAll(/\s\s+/g," ")!==e.title.toLowerCase().replaceAll("\n"," ").replaceAll(/\s\s+/g," "))return{note:null};if(!r.message.items[0].event||!r.message.items[0].event.name)return{note:null};info("Found a CrossRef match");var i=r.message.items[0].event.name.trim();return{venue:i,note:`Accepted @ ${i} -- [crossref.org]`}}catch(e){return logError("[Crossref]",e),{note:null}}},tryDBLP=async e=>{try{var t,r,i,a,n=encodeURI(e.title),o=await fetch(`https://dblp.org/search/publ/api?q=${n}&format=json`).then(e=>e.json());if(o.result&&o.result.hits&&o.result.hits.hit&&o.result.hits.hit.length)for(const s of o.result.hits.hit.sort((e,t)=>parseInt(e.info.year,10)-parseInt(t.info.year,10)))if(decodeHtml(s.info.title.toLowerCase().replaceAll("\n"," ").replaceAll(".","").replaceAll(/\s\s+/g," "))===e.title.toLowerCase().replaceAll("\n"," ").replaceAll(".","").replaceAll(/\s\s+/g," ")&&"CoRR"!==s.info.venue)return info("Found a DBLP match"),t=await fetchText(s.info.url+".bib"),r=miniHash(s.info.venue),await readJournalAbbreviations(),i=(global.journalAbbreviations[r]??s.info.venue).trim(),a=s.info.year,s.info.url,{venue:i,note:`Accepted @ ${i} ${a} -- [dblp.org]`,bibtex:t};return{note:null}}catch(e){return logError("[DBLP]",e),{note:null}}},trySemanticScholar=async t=>{try{var e=await fetchJSON(`https://api.semanticscholar.org/graph/v1/paper/search?query=${encodeURI(t.title)}&fields=title,venue,year,authors,externalIds,url&limit=50`);if(e&&e.data&&0<e.data.length)for(const s of e.data)if(miniHash(s.title)===miniHash(t.title)&&Math.abs(s.year-t.year)<3&&s.venue&&!s.venue.toLowerCase().includes("arxiv")){info("Found a Semantic Scholar match");let e=s.venue.trim().replace(/^\d{4}/,"").trim().capitalize(!0);e.indexOf(" ")<0&&(e=e.toUpperCase());var r=(s.year+"").trim(),i=`Accepted @ ${e} (${r}) -- [semanticscholar.org]`,a=s.authors.map(e=>e.name).join(" and "),n=s.externalIds.DOI,o=bibtexToString({entryType:"article",citationKey:miniHash(s.authors[0].name.split(" ").last())+r+firstNonStopLowercase(t.title),title:t.title,author:a,journal:e,year:r,doi:n,bibSource:"Semantic Scholar "+s.url});return{venue:e,note:i,bibtex:o}}}catch(e){logError("[SemanticScholar]",e)}},tryGoogleScholar=async e=>{return await sendMessageToBackground({type:"google-scholar",paper:e})},tryPreprintMatch=async(e,t=!1)=>{let r,i,a,n;var o,s,l={},c=["DBLP","SemanticScholar","CrossRef","GoogleScholar"],p=[silentPromiseTimeout(tryGoogleScholar(e)),silentPromiseTimeout(trySemanticScholar(e)),silentPromiseTimeout(tryCrossRef(e)),silentPromiseTimeout(tryDBLP(e))];t&&(p.push(silentPromiseTimeout(tryPWCMatch(e))),c.push("PapersWithCode"));for([o,s]of Object.entries(c)){if(l[s]=await p[o],{note:r,venue:i,bibtex:a}=l[s]??{},r)break;log(`[${s}] No publication found`)}return t&&(l.hasOwnProperty(c="PapersWithCode")||(l[c]=await p[c]),l[c].codeLink)&&!e.codeLink&&(n=l[c].codeLink),{note:r,venue:i,bibtex:a,code:n}},initPaper=async e=>{e.note||(e.note=""),e.md=`[${e.title}](${e.pdfLink})`,e.tags=[],e.codeLink="",e.favorite=!1,e.favoriteDate="",e.addDate=(new Date).toJSON(),e.lastOpenDate=e.addDate,e.count=1,e.code={};for(const t in e)e.hasOwnProperty(t)&&"string"==typeof e[t]&&(e[t]=e[t].trim());return e=await autoTagPaper(e),validatePaper(e),e},autoTagPaper=async r=>{try{var e,i,a=await getStorage("autoTags");if(a&&a.length){let t=new Set;for(const n of a)n.tags?.length&&(n.title||n.author)&&(e=!n.title||new RegExp(n.title,"i").test(r.title),i=!n.author||new RegExp(n.author,"i").test(r.author),e)&&i&&n.tags.forEach(e=>t.add(e));r.tags=[...t].sort(),r.tags.length&&log("Automatically adding tags:",r.tags)}return r}catch(e){return log("Error auto-tagging:",e),log("Paper:",r),r}},makePaper=async(e,t)=>{let r;if(e.arxiv)(r=await makeArxivPaper(t))&&(r.source="arxiv");else if(e.neurips)(r=await makeNeuripsPaper(t))&&(r.source="neurips");else if(e.cvf)(r=await makeCVFPaper(t))&&(r.source="cvf");else if(e.openreview)(r=await makeOpenReviewPaper(t))&&(r.source="openreview");else if(e.biorxiv)(r=await makeBioRxivPaper(t))&&(r.source="biorxiv");else if(e.pmlr)(r=await makePMLRPaper(t))&&(r.source="pmlr");else if(e.acl)(r=await makeACLPaper(t))&&(r.source="acl");else if(e.pnas)(r=await makePNASPaper(t))&&(r.source="pnas");else if(e.nature)(r=await makeNaturePaper(t))&&(r.source="nature");else if(e.acs)(r=await makeACSPaper(t))&&(r.source="acs");else if(e.iop)(r=await makeIOPPaper(t))&&(r.source="iop");else if(e.jmlr)(r=await makeJMLRPaper(t))&&(r.source="jmlr");else if(e.pmc)(r=await makePMCPaper(t))&&(r.source="pmc");else if(e.ijcai)(r=await makeIJCAIPaper(t))&&(r.source="ijcai");else if(e.acm)(r=await makeACMPaper(t))&&(r.source="acm");else if(e.ieee)(r=await makeIEEEPaper(t))&&(r.source="ieee");else if(e.springer)(r=await makeSpringerPaper(t))&&(r.source="springer");else if(e.aps)(r=await makeAPSPaper(t))&&(r.source="aps");else if(e.wiley)(r=await makeWileyPaper(t))&&(r.source="wiley");else if(e.sciencedirect)(r=await makeScienceDirectPaper(t))&&(r.source="sciencedirect");else if(e.science)(r=await makeSciencePaper(t))&&(r.source="science");else if(e.frontiers)(r=await makeFrontiersPaper(t))&&(r.source="frontiers");else if(e.ihep)(r=await makeIHEPPaper(t))&&(r.source="ihep");else if(e.plos)(r=await makePLOSPaper(t))&&(r.source="plos");else{if(!e.rsc)throw new Error("Unknown paper source: "+JSON.stringify({is:e,url:t}));(r=await makeRSCPaper(t))&&(r.source="rsc")}if(void 0!==r)return initPaper(r)},findFuzzyPaperMatch=(e,t)=>{t=miniHash(t.title);return e.hasOwnProperty(t)?(e=e[t]).find(t=>!global.preprintSources.some(e=>t.toLowerCase().startsWith(e)))||e[0]:null};"undefined"!=typeof module&&null!=module.exports&&(dummyModule=module,dummyModule.exports={autoTagPaper:autoTagPaper,decodeHtml:decodeHtml,fetchArxivXML:fetchArxivXML,fetchCrossRefDataForDoi:fetchCrossRefDataForDoi,fetchCvfHTML:fetchCvfHTML,fetchDom:fetchDom,getOpenReviewForumJSON:getOpenReviewForumJSON,getOpenReviewNoteJSON:getOpenReviewNoteJSON,fetchSemanticScholarDataForDoi:fetchSemanticScholarDataForDoi,fetchText:fetchText,findACLValue:findACLValue,findFuzzyPaperMatch:findFuzzyPaperMatch,flipAndAuthors:flipAndAuthors,flipAuthor:flipAuthor,initPaper:initPaper,makeACLPaper:makeACLPaper,makeACMPaper:makeACMPaper,makeACSPaper:makeACSPaper,makeAPSPaper:makeAPSPaper,makeArxivPaper:makeArxivPaper,makeBioRxivPaper:makeBioRxivPaper,makeCVFPaper:makeCVFPaper,makeIEEEPaper:makeIEEEPaper,makeIJCAIPaper:makeIJCAIPaper,makeIOPPaper:makeIOPPaper,makeJMLRPaper:makeJMLRPaper,makeNaturePaper:makeNaturePaper,makeNeuripsPaper:makeNeuripsPaper,makeOpenReviewBibTex:makeOpenReviewBibTex,makeOpenReviewPaper:makeOpenReviewPaper,makePaper:makePaper,makePMCPaper:makePMCPaper,makePMLRPaper:makePMLRPaper,makePNASPaper:makePNASPaper,makePubMedPaper:makePubMedPaper,makePLOSPaper:makePLOSPaper,makeSpringerPaper:makeSpringerPaper,makeWileyPaper:makeWileyPaper,tryCrossRef:tryCrossRef,tryDBLP:tryDBLP,tryPreprintMatch:tryPreprintMatch,tryPWCMatch:tryPWCMatch,trySemanticScholar:trySemanticScholar});