...
`,u+=`url={${e}},
`,u+=`year={${l}}
`,u+="}";return{author:r,bibtex:u=bibtexToString(u),conf:p,id:c,key:s,note:d,pdfLink:i,title:o,venue:"NeurIPS",year:l}},makeCVFPaper=async e=>{const a=await fetchCvfHTML(e),n=(new DOMParser).parseFromString(a.replaceAll("\n",""),"text/html");var t=n.getElementById("papertitle").innerText.trim(),o=n.querySelector("#authors i").innerText.split(",").map(e=>e.trim()).join(" and "),{year:r,id:i,conf:l}=parseCVFUrl(e);let s="";if(e.endsWith(".pdf"))s=e;else{const d=Array.from(n.getElementsByTagName("a")).filter(e=>"pdf"===e.innerText)[0].getAttribute("href");d.startsWith("../")&&(d=d.replaceAll("../","")),d.startsWith("/")||(d="/"+d),s="http://openaccess.thecvf.com"+d}var c=`Accepted @ ${l} `+r;const p=bibtexToString(n.querySelector(".bibref").innerText);e=p.split("{")[1].split(",")[0];return{author:o,bibtex:p,conf:l,id:i,key:e,note:c,pdfLink:s,title:t,venue:l,year:r}},makeOpenReviewBibTex=(e,a)=>{var n=e.content.title,t=e.content.authors.join(" and "),o=e.cdate?new Date(e.cdate).getFullYear():"0000";e.cdate||log("makeOpenReviewBibTex: no cdate found in",e);var r=e.content.authors[0].split(" ").reverse()[0];r+=o;e="";return e+=`@inproceedings{${r+=firstNonStopLowercase(n)},
`,e+=`title={${n}},
`,e+=`author={${t}},
`,e+=`year={${o}},
`,e+=`url={${a}},
`,e+="}"},makeOpenReviewPaper=async e=>{var a=await fetchOpenReviewNoteJSON(e),n=await fetchOpenReviewForumJSON(e),t=a.notes[0],o=n.notes,r=t.content.title,i=t.content.authors.join(" and "),l=bibtexToString(t.content._bibtex||makeOpenReviewBibTex(t,e)),a=bibtexToObject(l),n=a.citationKey,a=a.year;let s;s=t.pdf?"https://openreview.net/pdf?id="+t.id:(t.html||e).replace("/forum?id=","/pdf?id=");const c=t.invitation.split("/");let p=c[0].split(".")[0],d=c.slice(1).join("/").split("-")[0].replaceAll("/"," ").replace(" Conference",""),u=p,h=!1;global.overrideORConfs.hasOwnProperty(p)&&(u=global.overrideORConfs[p],h=!0),h&&(d=d.replace(u,""),p=u);e=(p+" "+d).replace(/ \d\d\d\d/g,"").replace(/\s\s+/g," "),t=`OR-${p}-${a}_`+t.id;let m,f,g;m=o.filter(e=>e&&e.content&&-1<["Final Decision","Paper Decision","Acceptance Decision"].indexOf(e.content.title));let y="";return m&&0<m.length&&(f=m[0].content.decision.split(" ").map((e,a)=>0===a?e+"ed":e).join(" "),g=f+` @ ${e} `+a,f.toLowerCase().indexOf("rejected")<0&&(y=e)),"Anonymous"===i&&(g=`Under review @ ${e} ${a} (${(new Date).toLocaleDateString()})`),{author:i,bibtex:l,conf:e,id:t,key:n,note:g,pdfLink:s,title:r,venue:y,year:a}},makeBioRxivPaper=async e=>{var a=e.replace(".full.pdf",""),n="https://api.biorxiv.org//details/biorxiv/"+e.split("/").slice(-2).join("/").replace(".full.pdf","").split("v")[0];const t=await fetch(n).then(e=>e.json());if("ok"!==t.messages[0].status)throw new Error(n+" returned "+t.messages[0].status);const o=t.collection.reverse()[0],r=await fetch(a),i=await r.text(),l=(new DOMParser).parseFromString(i.replaceAll("\n",""),"text/html");var s=l.querySelector(".bibtext a").getAttribute("href");const c=bibtexToString(await(await fetch(s)).text());var p=extractAuthor(c),d=await parseIdFromUrl(e),n=c.split("\n")[0].split("{")[1].replace(",","").trim(),a=cleanBiorxivURL(e)+".full.pdf",s=o.title,e=o.date.split("-")[0];return{author:p,bibtex:c,conf:"BioRxiv",id:d,key:n,note:"",pdfLink:a,title:s,venue:"",year:e}},makePMLRPaper=async e=>{var a=e.split("/").reverse()[0].split(".")[0],n=await parseIdFromUrl(e);const t=e.includes(".html")?e:e.split("/").slice(0,-2).join("/")+`/${a}.html`;var o=t.replace(".html","")+`/${a}.pdf`;const r=await fetchDom(t);r.getElementById("button-bibtex1").getAttribute("onclick").match(/https.+\.bib/)[0];const i=r.getElementById("bibtex").innerText.replaceAll("\t"," ").replaceAll(/\s\s+/g," ");let l=i;for(const u of i.match(/,\ ?\w+ ?= ?{/g))l=l.replace(u,u.replace(", ",",\n    ").replace(" = ","="));l.endsWith("}}")&&(l=l.slice(0,-2)+"}\n}"),l=bibtexToString(l);var s=extractAuthor(l),e=r.getElementsByTagName("h1")[0].innerText,c=extractBibtexValue(l,"year");let p=extractBibtexValue(l,"booktitle").replaceAll("Proceedings of the",""),d=p;note="Accepted @ "+p+`(${c})`;for(const h in global.overridePMLRConfs)if(p.includes(h)){d=global.overridePMLRConfs[h],p=d+" "+c,note="Accepted @ "+p;break}return{author:s,bibtex:l,conf:p,id:n,key:a,note:note,pdfLink:o,title:e,venue:d,year:c}},findACLValue=(e,a)=>{return Array.from(e.querySelectorAll("dt")).filter(e=>e.innerText.includes(a))[0].nextElementSibling.innerText},makeACLPaper=async e=>{e=e.replace(".pdf","");const a=await fetchDom(e);var n=a.getElementById("citeBibtexContent");if(n){var t=a.getElementById("title").innerText,o=bibtexToString(n.innerText),r=bibtexToJson(o)[0];const s=r.entryTags;var i=s.year,l=s.author.replace(/\s+/g," ").split(" and ").map(e=>e.split(",").map(e=>e.trim()).reverse().join(" ")).join(" and "),e=r.citationKey,n=findACLValue(a,"Venue"),r=findACLValue(a,"PDF");return{author:l,bibtex:o,conf:n,id:`ACL-${n}-${i}_`+findACLValue(a,"Anthology ID"),key:e,note:`Accepted @ ${n} `+i,pdfLink:r,title:t,venue:n,year:i}}},makePNASPaper=async e=>{e=e.replace(".full.pdf","").replace(/\/doi\/e?pdf\//,"/doi/abs/");const a=await fetchDom(e);var n=a.getElementsByTagName("h1")[0].innerText,t=Array.from(a.querySelectorAll(".authors span[property='author'] a:not([property='email']):not(.orcid-id)")).filter(e=>!e.getAttribute("href").includes("mailto:")).map(e=>e.innerText).join(" and "),o=a.querySelector("span[property='datePublished']").innerText.match(/\d{4}/g)[0],r=`PNAS-${o}_`+(e.endsWith("/")?e.split("/").slice(-2):e.split("/").slice(-1))[0];const i=e.includes("/doi/pdf/")||e.includes("/doi/epdf/")?e.replace("/doi/epdf/","/doi/pdf/"):e.replace("/doi/abs/","/doi/pdf/").replace("/doi/full/","/doi/pdf/");var l=Array.from(a.querySelector(".self-citation").getElementsByTagName("a")).map(e=>e.getAttribute("href")).filter(e=>e.includes("https://doi.org"))[0].split("/").slice(-2).join("/"),e="doi:"+l;return{author:t,bibtex:bibtexToString(`@article{${e}, author={${t}}, title={${n}}, journal = {Proceedings of the National Academy of Sciences}, year={${o}}, doi={${l}}, eprint={${i}}, URL={${i.replace("/doi/pdf/","/doi/abs/")}} }`),id:r,key:e,note:`Published @ PNAS (${o})`,pdfLink:i,title:n,venue:"PNAS",year:o}},makeNaturePaper=async e=>{var a=(e=e.replace(".pdf","").split("#")[0])+".pdf",n=e.split("/").reverse()[0];const t=await fetchDom(e);var o=t.querySelector("h1.c-article-title").innerText;const r=Array.from(t.querySelectorAll("ul.c-article-author-list li")).map(e=>e.innerText.replace(/(\ ?,)|&|…|\d/g,"").split(/orcid/i)[0].trim()).filter(e=>0<e.length).join(" and ");var i=t.querySelector(".c-article-info-details").innerText.match(/\(\d{4}\)/)[0].replace(/\(|\)/g,""),l=t.querySelector(".c-article-info-details [data-test]").innerText,s="Nature_"+n,c=document.querySelector(".c-bibliographic-information__citation").innerText.split("https://doi.org/")[1],e="https://doi.org/"+c,n=""+r.split(" ")[1]+i+firstNonStopLowercase(o),e=bibtexToString(`@article{${n}, author={${r}}, title={${o}}, journal = {${l}}, year={${i}}, doi={${c}}, url={${e}} }`);return{author:r,bibtex:e,id:s,key:n,note:`Published @ ${l} (${i})`,pdfLink:a,title:o,venue:l,year:i}},makeACSPaper=async e=>{const a=(e=e.replace("pubs.acs.org/doi/pdf/","pubs.acs.org/doi/").split("?")[0]).split("/doi/")[1];var n=`https://pubs.acs.org/action/downloadCitation?doi=${encodeURIComponent(a)}&include=cit&format=bibtex&direct=true`,t=(await(await fetch(n)).text()).trim();const o=bibtexToObject(t);var r=o.author.replaceAll("\n","").trim(),i=o.title.trim(),l=o.year.trim(),s=o.citationKey.trim(),n=e.replace("/doi/","/doi/pdf/"),e=`Published @ ${o.journal} (${o.year})`;return{author:r,bibtex:t,id:"ACS_"+a.replaceAll(".","").replaceAll("/",""),key:s,note:e,pdfLink:n,title:i,venue:o.journal,year:l}},makeIOPPaper=async e=>{(e=e.split("#")[0]).endsWith("/pdf")&&(e=e.slice(0,-4));const a=await fetchDom(e);var n=Array.from(a.querySelectorAll(".btn-multi-block a")).filter(e=>"BibTeX"===e.innerText).map(e=>e.getAttribute("href"))[0],t="https://"+parseUrl(e).host+n,o=(await(await fetch(t)).text()).trim();const r=bibtexToObject(o);var i=r.author.replaceAll("\n","").trim(),l=r.title.trim(),s=r.year.trim(),c=r.citationKey.trim(),p=e+"/pdf",n=r.journal,t=`Published @ ${n} (${s})`;const d=e.split("/article/").reverse()[0];return{author:i,bibtex:o,id:"IOPscience_"+d.replaceAll(".","").replaceAll("/",""),key:c,note:t,pdfLink:p,title:l,venue:n,year:s}},tryCrossRef=async e=>{try{var a="https://api.crossref.org/works?rows=1&mailto=schmidtv%40mila.quebec&select=event%2Ctitle&query.title="+encodeURI(e.title);const t=await fetch(a).then(e=>e.json());if("ok"!==t.status)return log(`[PM][Crossref] ${a} returned `+t.message.status),{note:null};if(0===t.message.items.length)return{note:null};if(t.message.items[0].title[0].toLowerCase().replaceAll("\n"," ").replaceAll(/\s\s+/g," ")!==e.title.toLowerCase().replaceAll("\n"," ").replaceAll(/\s\s+/g," "))return{note:null};if(!t.message.items[0].event||!t.message.items[0].event.name)return{note:null};info("Found a CrossRef match");var n=t.message.items[0].event.name.trim();return{venue:n,note:`Accepted @ ${n} -- [crossref.org]`}}catch(e){return log("[PM][Crossref]",e),{note:null}}},tryDBLP=async e=>{try{var a=encodeURI(e.title);const o=await fetch(`https://dblp.org/search/publ/api?q=${a}&format=json`).then(e=>e.json());if(!(o.result&&o.result.hits&&o.result.hits.hit&&o.result.hits.hit.length))return log("[PM][DBLP] No hits found"),{note:null};for(const r of o.result.hits.hit.sort((e,a)=>parseInt(e.info.year,10)-parseInt(a.info.year,10)))if(decodeHtml(r.info.title.toLowerCase().replaceAll("\n"," ").replaceAll(".","").replaceAll(/\s\s+/g," "))===e.title.toLowerCase().replaceAll("\n"," ").replaceAll(".","").replaceAll(/\s\s+/g," ")&&"CoRR"!==r.info.venue){info("Found a DBLP match");var n=r.info.venue.toLowerCase().replaceAll(".","").trim(),t=(global.journalAbbreviations[n]||r.info.venue).trim();return{venue:t,note:`Accepted @ ${t} ${r.info.year} -- [dblp.org]
`+r.info.url}}return log("[PM][DBLP] No match found"),{note:null}}catch(e){return log("[PM][DBLP]",e),{note:null}}},tryPreprintMatch=async e=>{let a="",n="";return dblpMatch=await tryDBLP(e),a=dblpMatch.note,n=dblpMatch.venue,a||(crossRefMatch=await tryCrossRef(e),a=crossRefMatch.note,n=crossRefMatch.venue),{note:a,venue:n}},initPaper=async e=>{e.note||(e.note=""),e.md=`[${e.title}](${e.pdfLink})`,e.tags=[],e.codeLink="",e.favorite=!1,e.favoriteDate="",e.addDate=(new Date).toJSON(),e.lastOpenDate=e.addDate,e.count=1,e.code={};for(const a in e)e.hasOwnProperty(a)&&"string"==typeof e[a]&&(e[a]=e[a].trim());return e=await autoTagPaper(e),validatePaper(e),e},autoTagPaper=async n=>{try{var e,t,o=await getStorage("autoTags");if(!o||!o.length)return n;let a=new Set;for(const r of o)r.tags?.length&&(r.title||r.author)&&(e=!r.title||new RegExp(r.title,"i").test(n.title),t=!r.author||new RegExp(r.author,"i").test(n.author),e&&t&&r.tags.forEach(e=>a.add(e)));return n.tags=Array.from(a).sort(),n.tags.length&&log("Automatically adding tags:",n.tags),n}catch(e){return log("Error auto-tagging:",e),log("Paper:",n),n}},makePaper=async(e,a,n)=>{let t;if(e.arxiv)t=await makeArxivPaper(n),t.source="arxiv";else if(e.neurips)t=await makeNeuripsPaper(a),t.source="neurips";else if(e.cvf)t=await makeCVFPaper(a),t.source="cvf";else if(e.openreview)t=await makeOpenReviewPaper(a),t.source="openreview";else if(e.biorxiv)t=await makeBioRxivPaper(a),t.source="biorxiv";else if(e.pmlr)t=await makePMLRPaper(a),t.source="pmlr";else if(e.acl)t=await makeACLPaper(a),t&&(t.source="pmlr");else if(e.pnas)t=await makePNASPaper(a),t&&(t.source="pnas");else if(e.nature)t=await makeNaturePaper(a),t&&(t.source="nature");else if(e.acs)t=await makeACSPaper(a),t&&(t.source="acs");else{if(!e.iop)throw new Error("Unknown paper source: "+JSON.stringify({is:e,url:a,id:n}));t=await makeIOPPaper(a),t&&(t.source="iop")}if(void 0!==t)return initPaper(t)},findFuzzyPaperMatch=e=>{for(const n in global.state.papers)if("__dataVersion"!==n){var a=global.state.papers[n];if(Math.abs(a.title.length-e.title.length)<global.fuzzyTitleMatchMinDist)if(levenshtein(a.title,e.title)<global.fuzzyTitleMatchMinDist)return a.id}return null};